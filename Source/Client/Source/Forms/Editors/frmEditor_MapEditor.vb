...
#Region "Drawing"
    ...
    Public Sub EditorMap_DrawTileset()
        ...
        TilesetWindow.Clear(Microsoft.Xna.Framework.Color.Black)
        ...
        Dim rec2 As New RectangleShape With {
            .OutlineColor = New Microsoft.Xna.Framework.Color(Microsoft.Xna.Framework.Color.Red),
        ...
        rec2.Size = New Vector2(EditorTileWidth * PicX, EditorTileHeight * PicY)

        rec2.Position = New Vector2((EditorTileSelStart.X * PicX - picbackleft), (EditorTileSelStart.Y * PicY - picbacktop))
    ...
    Friend Sub DrawTileOutline()
        If tabpages.SelectedTab Is tpDirBlock Then Exit Sub

        Dim destSize, destPos As Vector2
        Dim srcRect As New Rectangle(0, 0, PicX, PicY)
        Dim thickness = 0.6F
        Dim border = Color.Blue

        If tabpages.SelectedTab Is tpAttributes Then
            destSize = New Vector2(srcRect.Width, srcRect.Height)
        Else

            If EditorTileWidth = 1 AndAlso EditorTileHeight = 1 Then
                RenderSprite(TileSetSprite(cmbTileSets.SelectedIndex + 1), GameWindow, Graphics.ConvertMapX(CurX * PicX), Graphics.ConvertMapY(CurY * PicY), EditorTileSelStart.X * PicX, EditorTileSelStart.Y * PicY, rec.Width, rec.Height)
            ...
            Else
                If cmbAutoTile.SelectedIndex > 0 Then
                    ...
                    destSize = New Vector2(srcRect.Width, srcRect.Height)
                Else
                    RenderSprite(TileSetSprite(cmbTileSets.SelectedIndex + 1), GameWindow, Graphics.ConvertMapX(CurX * PicX), Graphics.ConvertMapY(CurY * PicY), EditorTileSelStart.X * PicX, EditorTileSelStart.Y * PicY, EditorTileSelEnd.X * PicX, EditorTileSelEnd.Y * PicY)

                    destSize = New Vector2(EditorTileSelEnd.X * PicX, EditorTileSelEnd.Y * PicY)
                End If
        ...
        destPos = New Vector2(Graphics.ConvertMapX(CurX * PicX), Graphics.ConvertMapY(CurY * PicY))
