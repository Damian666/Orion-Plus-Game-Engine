...
#End Region
    ...
    Friend Sub InitAutotiles()
        ...
        ' Procedure used to cache autotile positions. All positioning is independant from the
        ' tileset. Calculations are convoluted and annoying. Maths is not my strong point. Luckily
        ' we're caching them so it's a one-off thing when the map is originally loaded. As such
        ' optimisation isn't an issue. For simplicity's sake we cache all subtile SOURCE positions in
        ' to an array. We also give letters to each subtile for easy rendering tweaks. ;] First, we
        ' need to re-size the array
        ...
        ' Inner tiles (Top right subtile region) NW - a
        AutoIn(1).X = 32
        AutoIn(1).Y = 0
        ' NE - b
        AutoIn(2).X = 48
        ...
        AutoIn(4).X = 48
        AutoIn(4).Y = 16
        ' Outer Tiles - NW (bottom subtile region) NW - e
        AutoNw(1).X = 0
        AutoNw(1).Y = 32
        ' NE - f
        AutoNw(2).X = 16
        ...
        AutoNw(4).X = 16
        AutoNw(4).Y = 48
        ' Outer Tiles - NE (bottom subtile region) NW - i
        AutoNe(1).X = 32
        AutoNe(1).Y = 32
        ' NE - g
        AutoNe(2).X = 48
        ...
        AutoNe(4).X = 48
        AutoNe(4).Y = 48
        ' Outer Tiles - SW (bottom subtile region) NW - m
        AutoSw(1).X = 0
        AutoSw(1).Y = 64
        ' NE - n
        AutoSw(2).X = 16
        ...
        AutoSw(4).X = 16
        AutoSw(4).Y = 80
        ' Outer Tiles - SE (bottom subtile region) NW - q
        AutoSe(1).X = 32
        AutoSe(1).Y = 64
        ' NE - r
        AutoSe(2).X = 48
    ...
    Friend Sub CalculateAutotile(x As Integer, y As Integer, layerNum As Integer)
        ' Right, so we've split the tile block in to an easy to remember collection of letters. We
        ' now need to do the calculations to find out which little lettered block needs to be
        ' rendered. We do this by reading the surrounding tiles to check for matches. First we check
        ' to make sure an autotile situation is actually there. Then we calculate exactly which
        ' situation has arisen. The situations are "inner", "outer", "horizontal", "vertical" and
        ' "fill". Exit out if we don't have an autotile
