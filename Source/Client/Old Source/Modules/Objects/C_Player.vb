...
Module C_Player
    ...
    Function IsPlaying(index As Integer) As Boolean
        ...
        If GetPlayerName(index).Length > 0 Then
            IsPlaying = True
        End If
                ...
                For i = 1 To Map.CurrentEvents
                    If Map.MapEvents(i).Visible = 1 Then
                        If Map.MapEvents(i).X = x AndAlso Map.MapEvents(i).Y = y Then
                            ...
                            buffer.WriteInt32(ClientPacket.CEvent)
                            buffer.WriteInt32(i)
    ...
    Friend Sub PlayerCastSkill(skillslot As Integer)
        ...
        If GetPlayerVital(Myindex, VitalType.MP) < Skill(PlayerSkills(skillslot)).MpCost Then
            AddText("Not enough MP to cast " & Skill(PlayerSkills(skillslot)).Name.Trim & ".", QColorType.AlertColor)
            Exit Sub
        End If
        ...
        If PlayerSkills(skillslot) > 0 Then
            If GetTickCount() > Player(Myindex).AttackTimer + 1000 Then
                If Player(Myindex).Moving = 0 Then
                    buffer.WriteInt32(ClientPacket.CCast)
