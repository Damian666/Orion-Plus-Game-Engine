...
Module C_Text
    ...
    Friend Sub DrawPlayerName(index As Integer)
        ...
        ' Draw name
        DrawText(textX, textY, name.Trim, color, backcolor, GameWindow)
    End Sub
    ...
    Friend Sub DrawNpcName(mapNpcNum As Integer)
        ...
        textX = ConvertMapX(MapNpc(mapNpcNum).X * PicX) + MapNpc(mapNpcNum).XOffset + (PicX \ 2) - GetTextWidth((Npc(npcNum).Name.Trim)) / 2
        ...
        ' Draw name
        DrawText(textX, textY, Npc(npcNum).Name.Trim, color, backcolor, GameWindow)
    End Sub
    ...
    Friend Sub DrawEventName(index As Integer)
        ...
        name = Map.MapEvents(index).Name.Trim

        ' calc pos
        textX = ConvertMapX(Map.MapEvents(index).X * PicX) + Map.MapEvents(index).XOffset + (PicX \ 2) - GetTextWidth(name.Trim) \ 2
            ...
            If Player(Myindex).PlayerQuest(i).Status = QuestStatusType.NotStarted OrElse Player(Myindex).PlayerQuest(i).Status = QuestStatusType.Repeatable OrElse (Player(Myindex).PlayerQuest(i).Status = QuestStatusType.Completed AndAlso Quest(i).Repeat = 1) Then
                ...
                If Map.MapEvents(index).Questnum = i Then
                    ...
                    If Quest(i).Repeat = 1 Then
                        DrawText(textX, textY, name.Trim, Color.White, backcolor, GameWindow)
                    Else
                        DrawText(textX, textY, name.Trim, color, backcolor, GameWindow)
                    End If
                    Exit For
        ...
        Select Case ActionMsg(index).Type
            Case ActionMsgType.Static
                ...
                If ActionMsg(index).Y > 0 Then
                    x = ActionMsg(index).X + CInt(PicX \ 2) - ((ActionMsg(index).Message.Trim.Length \ 2) * 8)
                    y = ActionMsg(index).Y - CInt(PicY \ 2) - 2
                Else
                    x = ActionMsg(index).X + CInt(PicX \ 2) - ((ActionMsg(index).Message.Trim.Length \ 2) * 8)
                    y = ActionMsg(index).Y - CInt(PicY \ 2) + 18
                End If
            ...
            Case ActionMsgType.Scroll
                ...
                If ActionMsg(index).Y > 0 Then
                    x = ActionMsg(index).X + CInt(PicX \ 2) - ((ActionMsg(index).Message.Trim.Length \ 2) * 8)
                    y = ActionMsg(index).Y - CInt(PicY \ 2) - 2 - (ActionMsg(index).Scroll * 0.6)
                    ActionMsg(index).Scroll = ActionMsg(index).Scroll + 1
                Else
                    x = ActionMsg(index).X + CInt(PicX \ 2) - ((ActionMsg(index).Message.Trim.Length \ 2) * 8)
                    y = ActionMsg(index).Y - CInt(PicY \ 2) + 18 + (ActionMsg(index).Scroll * 0.6)
                    ActionMsg(index).Scroll = ActionMsg(index).Scroll + 1
                End If
            ...
            Case ActionMsgType.Screen
                ...
                For i = Byte.MaxValue To 1 Step -1
                    ...
                    End If
                Next
                x = (FrmGame.picscreen.Width \ 2) - ((ActionMsg(index).Message.Trim.Length \ 2) * 8)
                y = 425
    ...
    Friend Sub AddText(msg As String, color As Integer)
        ...
        Else
            For Each str As String In WordWrap(msg, MyChatWindowGfxInfo.Width - ChatboxPadding, WrapMode.Font)
                TxtChatAdd = TxtChatAdd & Environment.NewLine & str
    ...
    Friend SplitChars As Char() = New Char() {" "c, "-"c, Convert.ToChar(Keys.Tab)}
    ...
    Friend Function Explode(str As String, splitChars As Char()) As String()
        ...
        While True
            ...
            ' Dashes and the likes should stick to the word occuring before it. Whitespace doesn't
            ' have to.
    ...
    Friend Sub DrawChatBubble(index As Integer)
        ...
        With ChatBubble(index)
            ...
            For i = 0 To theArray.Count - 1
                DrawText(x - (GetTextWidth(theArray(i)) / 2), y2, theArray(i), ToSfmlColor(Drawing.ColorTranslator.FromOle(.Colour)), Color.Black, GameWindow)
                y2 = y2 + 12
            Next
