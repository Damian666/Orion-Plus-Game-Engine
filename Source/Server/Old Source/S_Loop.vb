...
Module modLoop
            ...
            If tick > tmr500 Then
                ...
                Dim playerhousing = (
                    ...
                    Where Player(p.Index).Character(p.player.CurChar).InHouse > 0 AndAlso
                          Network.IsPlaying(Player(p.Index).Character(p.player.CurChar).InHouse) AndAlso
    ...
    Sub UpdateSavePlayers()
        ...
        If GetPlayersOnline() > 0 Then
            ...
            Network.GlobalMsg("Saving all online players...")
        ...
        For i = 1 To GetPlayersOnline()
            ...
            If Network.IsPlaying(i) Then
                ...
                If GetPlayerVital(i, VitalType.SP) <> GetPlayerMaxVital(i, VitalType.SP) Then
                    ...
                    Network.SendVital(i, VitalType.SP)
                End If
            End If
                ...
                For x1 = 0 To Map(mapNum).MaxX
                    For y1 = 0 To Map(mapNum).MaxY
                        If Map(mapNum).Tile(x1, y1).Type = TileType.Key AndAlso TempTile(mapNum).DoorOpen(x1, y1) = True Then
                            ...
                            Network.SendMapKeyToMap(mapNum, x1, y1, 0)
                        End If
                    Next
                Next
            ...
            If PlayersOnMap(mapNum) = True Then
                ...
                For x = 1 To MAX_MAP_NPCS
                    ...
                    If MapNpc(mapNum).Npc(x).SkillBuffer > 0 AndAlso Map(mapNum).Npc(x) > 0 AndAlso MapNpc(mapNum).Npc(x).Num > 0 Then
                        ...
                        End If
                    Else
                        ' ///////////////////////////////////////// // This is used for ATTACKING ON
                        ' SIGHT // ///////////////////////////////////////// Make sure theres a npc
                        ' with the map
                                                    ...
                                                    If distanceX <= n AndAlso distanceY <= n Then
                                                        If Npc(npcNum).Behaviour = NpcBehavior.AttackOnSight OrElse GetPlayerPK(i) = i Then
                                                            If Npc(npcNum).AttackSay.Trim.Length > 0 Then
                                                                Network.PlayerMsg(i, Npc(npcNum).Name.Trim & " says: " & Npc(npcNum).AttackSay.Trim, QColorType.SayColor)
                                                            End If
                                                ...
                                                Else
                                                    ...
                                                    If distanceX <= n AndAlso distanceY <= n Then
                                                        If Npc(npcNum).Behaviour = NpcBehavior.AttackOnSight OrElse GetPlayerPK(i) = True Then
                                                            If Npc(npcNum).AttackSay.Trim.Length > 0 Then
                                                                Network.PlayerMsg(i, CheckGrammar(Npc(npcNum).Name.Trim, 1) & " says, '" & Npc(npcNum).AttackSay.Trim & "' to you.", ColorType.Yellow)
                                                            End If
                        ...
                        ' ///////////////////////////////////////////// // This is used for NPC
                        ' walking/targetting // ///////////////////////////////////////////// Make
                        ' sure theres a npc with the map
                        If Map(mapNum).Npc(x) > 0 AndAlso MapNpc(mapNum).Npc(x).Num > 0 Then
                            ...
                            Else
                                ...
                                If Npc(npcNum).Behaviour <> NpcBehavior.ShopKeeper AndAlso Npc(npcNum).Behaviour <> NpcBehavior.Quest Then
                                    ...
                                    ElseIf targetTypes = TargetType.Pet Then
                                        If target > 0 Then
                                            If Network.IsPlaying(target) = True AndAlso GetPlayerMap(target) = mapNum AndAlso PetAlive(target) Then
                                                targetVerify = True
                    ...
                    ' ///////////////////////////////////////////// // This is used for npcs to
                    ' attack targets // ///////////////////////////////////////////// Make sure
                    ' theres a npc with the map
                    If Map(mapNum).Npc(x) > 0 AndAlso MapNpc(mapNum).Npc(x).Num > 0 Then
                        target = MapNpc(mapNum).Npc(x).Target
                        targetTypes = MapNpc(mapNum).Npc(x).TargetType
                    ...
                    ' //////////////////////////////////////////// // This is used for regenerating
                    ' NPC's HP // //////////////////////////////////////////// Check to see if we
                    ' want to regen some of the npc's hp
                    ...
                    ' //////////////////////////////////////////////////////// // This is used for
                    ' checking if an NPC is dead or not //
                    ' //////////////////////////////////////////////////////// Check if the npc is
                    ' dead or not
                    ...
                    ' ////////////////////////////////////// Check if we are supposed to spawn an npc
                    ' or not
    ...
    Friend Function HandlePlayerHouse(index As Integer) As Boolean
        ...
        Network.PlayerMsg(index, "Your visitation has ended. Possibly due to a disconnection. You are being warped back to your previous location.", ColorType.Yellow)
    ...
    Friend Sub CastSkill(index As Integer, skillSlot As Integer)
        ...
        ' Preventative checks
        If Not Network.IsPlaying(index) OrElse skillSlot <= 0 OrElse skillSlot > MAX_PLAYER_SKILLS OrElse Not HasSkill(index, skillId) Then Exit Sub

        ' Check if the player is able to cast the spell.
        If GetPlayerVital(index, VitalType.MP) < Skill(skillId).MpCost Then
            Network.PlayerMsg(index, "Not enough mana!", ColorType.BrightRed)
            Exit Sub
        ElseIf GetPlayerLevel(index) < Skill(skillId).LevelReq Then
            Network.PlayerMsg(index, String.Format("You must be level {0} to use this skill.", Skill(skillId).LevelReq), ColorType.BrightRed)
            Exit Sub
        ...
        ElseIf Not Skill(skillId).ClassReq = 0 AndAlso GetPlayerClass(index) <> Skill(skillId).ClassReq Then
            Network.PlayerMsg(index, String.Format("Only {0} can use this skill.", CheckGrammar((Classes(Skill(skillId).ClassReq).Name.Trim))), ColorType.BrightRed)
            Exit Sub
    ...
    Private Sub FinalizeCast(index As Integer, skillSlot As Integer, skillCost As Integer)
        ...
        Network.SendCooldown(index, skillSlot)
    End Sub
            ...
            Case 1, 3 ' self-cast AOE & targetted AOE
                ...
                Select Case Skill(skillnum).Type
                                ...
                                If GetPlayerMap(i) = mapNum Then
                                    If IsInRange(aoe, x, y, GetPlayerX(i), GetPlayerY(i)) Then
                                        If CanNpcAttackPlayer(npcNum, i) Then
                                            ...
                                            Network.PlayerMsg(i, Npc(MapNpc(mapNum).Npc(npcNum).Num).Name.Trim & " uses " & Skill(skillnum).Name.Trim & "!", ColorType.Yellow)
                    ...
                    Case SkillType.HealHp, SkillType.HealMp, SkillType.DamageMp
                        ...
                        didCast = True
                        For i = 1 To GetPlayersOnline()
                            If Network.IsPlaying(i) AndAlso GetPlayerMap(i) = GetPlayerMap(npcNum) Then
                                If IsInRange(aoe, x, y, GetPlayerX(i), GetPlayerY(i)) Then
                    ...
                    Case SkillType.DamageHp
                        If MapNpc(mapNum).Npc(npcNum).TargetType = Enums.TargetType.Player Then
                            If CanNpcAttackPlayer(npcNum, target) AndAlso vital > 0 Then
                                ...
                                Network.PlayerMsg(target, Npc(MapNpc(mapNum).Npc(npcNum).Num).Name.Trim & " uses " & Skill(skillnum).Name.Trim & "!", ColorType.Yellow)
    ...
    Friend Sub SkillNpc_Effect(vital As Byte, increment As Boolean, index As Integer, damage As Integer, skillnum As Integer, mapNum As Integer)
        ...
        If damage > 0 Then
            ...
            Network.SendActionMsg(mapNum, sSymbol & damage, color, ActionMsgType.Scroll, MapNpc(mapNum).Npc(index).X * 32, MapNpc(mapNum).Npc(index).Y * 32)
