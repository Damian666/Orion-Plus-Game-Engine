...
#Region "Database"
    ...
    Sub SavePet(petNum As Integer)
        ...
        filename = Environment.CurrentDirectory & "\data\pets\pet" & petNum & ".dat"
    ...
    Sub CheckPets()
        For i = 1 To MAX_PETS
            If Not File.Exists(Environment.CurrentDirectory & "\Data\pets\pet" & i & ".dat") Then
                SavePet(i)
            End If
        Next
    End Sub
...
#Region "Outgoing Packets"
    ...
    Sub SendPets(index As Integer)
        ...
        For i = 1 To MAX_PETS
            If Pet(i).Name.Trim.Length > 0 Then
    ...
    Sub SendUpdatePetToAll(petNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SUpdatePet)
    ...
    Friend Sub SendUpdatePlayerPet(index As Integer, ownerOnly As Boolean)
        ...
        buffer.WriteInt32(ServerPacket.SUpdatePlayerPet)
        ...
        If ownerOnly Then
            Network.SendDataTo(index, buffer.ToPacket)
        Else
            Network.SendDataToMap(GetPlayerMap(index), buffer.ToPacket)
        End If
    ...
    Sub SendPetAttack(index As Integer, mapNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPetAttack)
        buffer.WriteInt32(index)
    ...
    Sub SendPetXy(index As Integer, x As Integer, y As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPetXY)
        buffer.WriteInt32(index)
        buffer.WriteInt32(x)
        buffer.WriteInt32(y)
        Network.SendDataToMap(GetPlayerMap(index), buffer.ToPacket)
    ...
    Sub SendPetExp(index As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPetExp)
        buffer.WriteInt32(GetPetExp(index))
        buffer.WriteInt32(GetPetNextLevel(index))
        Network.SendDataTo(index, buffer.ToPacket)
        buffer.Dispose()
...
#Region "Incoming Packets"
    ...
    Sub Packet_RequestEditPet(index As Integer, ByRef data() As Byte)
        ...
        Dim buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SPetEditor)
        Network.SendDataTo(index, buffer.ToPacket)
                ...
                If GetPlayerMap(index) = GetPlayerMap(i) AndAlso GetPlayerX(i) = x AndAlso GetPlayerY(i) = y Then
                    If i = index Then
                        ...
                        If TempPlayer(index).PetTargetType = TargetType.Player AndAlso TempPlayer(index).PetTarget = i Then
                            ...
                            Network.PlayerMsg(index, "Your pet is no longer following you.", ColorType.BrightGreen)
                        Else
                            ...
                            Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " is now following you.", ColorType.BrightGreen)
                        End If
                    Else
                        ' Change target
                        If TempPlayer(index).PetTargetType = TargetType.Player AndAlso TempPlayer(index).PetTarget = i Then
                            ...
                            TempPlayer(index).PetTargetType = TargetType.None
                            ' send target to player
                            Network.PlayerMsg(index, "Your pet is no longer targetting " & GetPlayerName(i).Trim & ".", ColorType.BrightGreen)
                        Else
                            TempPlayer(index).PetTarget = i
                            TempPlayer(index).PetTargetType = TargetType.Player
                            ' send target to player
                            Network.PlayerMsg(index, "Your pet is now targetting " & GetPlayerName(i).Trim & ".", ColorType.BrightGreen)
                        End If
                    End If
                    Exit Sub
                End If
            ...
            If PetAlive(i) AndAlso i <> index Then
                If GetPetX(i) = x AndAlso GetPetY(i) = y Then
                    ...
                    If TempPlayer(index).PetTargetType = TargetType.Pet AndAlso TempPlayer(index).PetTarget = i Then
                        ...
                        TempPlayer(index).PetTargetType = TargetType.None
                        ' send target to player
                        Network.PlayerMsg(index, "Your pet is no longer targetting " & GetPlayerName(i).Trim & "'s " & GetPetName(i).Trim & ".", ColorType.BrightGreen)
                    Else
                        TempPlayer(index).PetTarget = i
                        TempPlayer(index).PetTargetType = TargetType.Pet
                        ' send target to player
                        Network.PlayerMsg(index, "Your pet is now targetting " & GetPlayerName(i).Trim & "'s " & GetPetName(i).Trim & ".", ColorType.BrightGreen)
                    End If
                    Exit Sub
                End If
            End If
        ...
        For i = 1 To MAX_MAP_NPCS
            If MapNpc(GetPlayerMap(index)).Npc(i).Num > 0 AndAlso MapNpc(GetPlayerMap(index)).Npc(i).X = x AndAlso MapNpc(GetPlayerMap(index)).Npc(i).Y = y Then
                If TempPlayer(index).PetTarget = i AndAlso TempPlayer(index).PetTargetType = TargetType.Npc Then
                    ...
                    TempPlayer(index).PetTargetType = TargetType.None
                    ' send target to player
                    Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & "'s target is no longer a " & Npc(MapNpc(GetPlayerMap(index)).Npc(i).Num).Name.Trim & "!", ColorType.BrightGreen)
                    Exit Sub
                Else
                    ' Change target
                    TempPlayer(index).PetTarget = i
                    ...
                    ' send target to player
                    Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & "'s target is now a " & Npc(MapNpc(GetPlayerMap(index)).Npc(i).Num).Name.Trim & "!", ColorType.BrightGreen)
                    Exit Sub
                End If
            End If
        Next
        ...
        If PetAlive(index) Then
            Select Case behaviour
                Case PetAttackBehaviourAttackonsight
                    ...
                    Network.SendActionMsg(GetPlayerMap(index), "Agressive Mode!", ColorType.White, 0, GetPetX(index) * 32, GetPetY(index) * 32, index)
                Case PetAttackBehaviourGuard
                    SetPetBehaviour(index, PetAttackBehaviourGuard)
                    Network.SendActionMsg(GetPlayerMap(index), "Defensive Mode!", ColorType.White, 0, GetPetX(index) * 32, GetPetY(index) * 32, index)
            End Select
        End If
    ...
    Sub Packet_UsePetStatPoint(index As Integer, ByRef data() As Byte)
        ...
        If GetPetPoints(index) > 0 Then
            ...
            If GetPetStat(index, pointType) >= 255 Then
                Network.PlayerMsg(index, "You cannot spend any more points on that stat for your pet.", ColorType.BrightRed)
                Exit Sub
            End If
            ...
            Network.SendActionMsg(GetPlayerMap(index), "+1 " & sMes, ColorType.White, 1, (GetPetX(index) * 32), (GetPetY(index) * 32))
        Else
            Exit Sub
        End If
...
#Region "Pet Functions"
                    ...
                    If GetPetBehaviour(playerindex) <> PetAttackBehaviourDonothing Then
                        ...
                        If Not TempPlayer(playerindex).PetStunDuration > 0 Then
                            ...
                            For i = 1 To Network.HighIndex
                                ...
                                If Network.IsPlaying(i) AndAlso i <> playerindex Then
                            ...
                            If GetPetBehaviour(playerindex) <> PetAttackBehaviourDonothing Then
                                ...
                                If targetTypes = TargetType.Player Then ' player
                                    ...
                                    If target > 0 Then
                                        ...
                                        If Network.IsPlaying(target) AndAlso GetPlayerMap(target) = mapNum Then
                                ...
                                ElseIf targetTypes = TargetType.Pet Then 'other pet
                                    If target > 0 Then
                                        If Network.IsPlaying(target) AndAlso GetPlayerMap(target) = mapNum AndAlso PetAlive(target) Then
                                            didWalk = False
                                            targetVerify = True
                        ...
                        ' //////////////////////////////////////////// // This is used for
                        ' regenerating Pet's HP // //////////////////////////////////////////// Check
                        ' to see if we want to regen some of the npc's hp
                        If Not TempPlayer(playerindex).PetstopRegen Then
                            If PetAlive(playerindex) AndAlso tickCount > givePetHpTimer + 10000 Then
                                If GetPetVital(playerindex, VitalType.HP) > 0 Then
                                    ...
                                    ' Check if they have more then they should and if so just set it
                                    ' to max
    ...
    Sub SummonPet(index As Integer)
        ...
        Network.PlayerMsg(index, "You summoned your " & GetPetName(index).Trim & "!", ColorType.BrightGreen)
        SendUpdatePlayerPet(index, False)
    End Sub

    Sub ReCallPet(index As Integer)
        Network.PlayerMsg(index, "You recalled your " & GetPetName(index).Trim & "!", ColorType.BrightGreen)
    ...
    Sub ReleasePet(index As Integer)
        ...
        Network.PlayerMsg(index, "You released your pet!", ColorType.BrightGreen)
    ...
    Sub AdoptPet(index As Integer, petNum As Integer)
        ...
        If GetPetNum(index) = 0 Then
            Network.PlayerMsg(index, "You have adopted a " & Pet(petNum).Name.Trim, ColorType.BrightGreen)
        Else
            Network.PlayerMsg(index, "You allready have a " & Pet(petNum).Name.Trim & ", release your old pet first!", ColorType.BrightGreen)
            Exit Sub
        End If
    ...
    Sub PetMove(index As Integer, mapNum As Integer, dir As Integer, movement As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPetMove)
        buffer.WriteInt32(index)
        buffer.WriteInt32(GetPetX(index))
        buffer.WriteInt32(GetPetY(index))
        buffer.WriteInt32(GetPetDir(index))
    ...
    Sub PetDir(index As Integer, dir As Integer)
        ...
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SPetDir)
        buffer.WriteInt32(index)
        ...
        Do While reachable = False
            ...
            For j = 0 To Map(mapNum).MaxY
                For i = 0 To Map(mapNum).MaxX
                    ...
                    'If j = 10 AndAlso i = 0 Then MessageBox.Show "hi!"
    ...
    Function GetPetDamage(index As Integer) As Integer
        ...
        If Network.IsPlaying(index) = False OrElse index <= 0 OrElse index > MAX_PLAYERS OrElse Not PetAlive(index) Then
            Exit Function
        End If
...
#Region "Pet > Npc"
    ...
    Friend Sub TryPetAttackNpc(index As Integer, mapNpcNum As Integer)
        ...
        If CanPetAttackNpc(index, mapNpcNum) Then
            ...
            Else
                Network.PlayerMsg(index, "Your pet's attack does nothing.", ColorType.BrightRed)
            End If
    ...
    Friend Function CanPetAttackNpc(attacker As Integer, mapnpcnum As Integer, Optional isSpell As Boolean = False) As Boolean
        ...
        If Network.IsPlaying(attacker) = False OrElse mapnpcnum <= 0 OrElse mapnpcnum > MAX_MAP_NPCS OrElse Not PetAlive(attacker) Then
            Exit Function
        End If
    ...
    Friend Sub PetAttackNpc(attacker As Integer, mapnpcnum As Integer, damage As Integer, Optional skillnum As Integer = 0, Optional overTime As Boolean = False)
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(attacker) = False OrElse mapnpcnum <= 0 OrElse mapnpcnum > MAX_MAP_NPCS OrElse damage < 0 OrElse Not PetAlive(attacker) Then
            Exit Sub
        End If

        mapNum = GetPlayerMap(attacker)
        npcnum = MapNpc(mapNum).Npc(mapnpcnum).Num
        name = Npc(npcnum).Name.Trim
        ...
        If damage >= MapNpc(mapNum).Npc(mapnpcnum).Vital(VitalType.HP) Then
            ...
            Network.SendActionMsg(GetPlayerMap(attacker), "-" & MapNpc(mapNum).Npc(mapnpcnum).Vital(VitalType.HP), ColorType.BrightRed, 1, (MapNpc(mapNum).Npc(mapnpcnum).X * 32), (MapNpc(mapNum).Npc(mapnpcnum).Y * 32))
            ...
            ' Now set HP to 0 so we know to actually kill them in the server loop (this prevents
                    ...
                    If GetPlayerMap(i) = mapNum Then
                        If TempPlayer(i).TargetType = TargetType.Npc Then
                            If TempPlayer(i).Target = mapnpcnum Then
                                ...
                                Network.SendTarget(i, 0, TargetType.None)
                            End If
                        End If
...
#Region "Npc > Pet"
    ...
    Friend Sub TryNpcAttackPet(mapNpcNum As Integer, index As Integer)
        ...
        If CanNpcAttackPet(mapNpcNum, index) Then
            ...
            If CanPetDodge(index) Then
                Network.SendActionMsg(mapNum, "Dodge!", ColorType.Pink, ActionMsgType.Scroll, (GetPetX(index) * 32), (GetPetY(index) * 32))
                Exit Sub
            End If
            ...
            If CanNpcCrit(npcnum) Then
                damage = damage * 1.5
                Network.SendActionMsg(mapNum, "Critical!", ColorType.BrightCyan, ActionMsgType.Scroll, (MapNpc(mapNum).Npc(mapNpcNum).X * 32), (MapNpc(mapNum).Npc(mapNpcNum).Y * 32))
            End If
        End If
    ...
    Function CanNpcAttackPet(mapNpcNum As Integer, index As Integer) As Boolean
        ...
        If mapNpcNum <= 0 OrElse mapNpcNum > MAX_MAP_NPCS OrElse Not Network.IsPlaying(index) OrElse Not PetAlive(index) Then
            Exit Function
        End If
        ...
        ' Make sure they are on the same map
        If Network.IsPlaying(index) AndAlso PetAlive(index) Then
            If npcnum > 0 Then
    ...
    Sub NpcAttackPet(mapnpcnum As Integer, victim As Integer, damage As Integer)
        ...
        ' Check for subscript out of range
        If mapnpcnum <= 0 OrElse mapnpcnum > MAX_MAP_NPCS OrElse Network.IsPlaying(victim) = False OrElse Not PetAlive(victim) Then
            Exit Sub
        End If
        ...
        name = Npc(MapNpc(mapNum).Npc(mapnpcnum).Num).Name.Trim
        ...
        If damage >= GetPetVital(victim, VitalType.HP) Then
            ' Say damage
            Network.SendActionMsg(GetPlayerMap(victim), "-" & GetPetVital(victim, VitalType.HP), ColorType.BrightRed, ActionMsgType.Scroll, (GetPetX(victim) * 32), (GetPetY(victim) * 32))

            ' kill pet
            Network.PlayerMsg(victim, "Your " & GetPetName(victim).Trim & " was killed by a " & Npc(MapNpc(mapNum).Npc(mapnpcnum).Num).Name.Trim & ".", ColorType.BrightRed)
            ReCallPet(victim)
        ...
        Else
            ...
            ' Say damage
            Network.SendActionMsg(GetPlayerMap(victim), "-" & damage, ColorType.BrightRed, ActionMsgType.Scroll, (GetPetX(victim) * 32), (GetPetY(victim) * 32))
            Network.SendBlood(GetPlayerMap(victim), GetPetX(victim), GetPetY(victim))
...
#End Region
    ...
    Function CanPetAttackPlayer(attacker As Integer, victim As Integer, Optional isSkill As Boolean = False) As Boolean
        ...
        If GetPlayerAccess(attacker) > AdminType.Monitor Then
            Network.PlayerMsg(attacker, "Admins cannot attack other players.", ColorType.Yellow)
            Exit Function
        End If

        ' Check to make sure the victim isn't an admin
        If GetPlayerAccess(victim) > AdminType.Monitor Then
            Network.PlayerMsg(attacker, "You cannot attack " & GetPlayerName(victim) & "!", ColorType.Yellow)
            Exit Function
        End If
        ...
        If TempPlayer(attacker).InParty > 0 AndAlso TempPlayer(victim).InParty > 0 Then
            If TempPlayer(attacker).InParty = TempPlayer(victim).InParty Then
                Network.PlayerMsg(attacker, "You can't attack another party member!", ColorType.Yellow)
                Exit Function
            End If
        End If
    ...
    Sub SendPetVital(index As Integer, vital As VitalType)
        ...
        buffer.WriteInt32(ServerPacket.SPetVital)
    ...
    Friend Sub BufferPetSkill(index As Integer, skillSlot As Integer)
        ...
        If TempPlayer(index).PetSkillCd(skillSlot) > GetTimeMs() Then
            Network.PlayerMsg(index, GetPetName(index).Trim & "'s Skill hasn't cooled down yet!", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        If GetPetVital(index, VitalType.MP) < mpCost Then
            Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " does not have enough mana!", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        If levelReq > GetPetLevel(index) Then
            Network.PlayerMsg(index, GetPetName(index).Trim & " must be level " & levelReq & " to cast this skill.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        If accessReq > GetPlayerAccess(index) Then
            Network.PlayerMsg(index, "You must be an administrator to cast this spell, even as a pet owner.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        Select Case skillCastType
            ...
            Case 2, 3 ' targeted & targeted AOE
                ...
                If Not target > 0 Then
                    ...
                    Else
                        Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " does not have a target.", ColorType.Yellow)
                    End If
                End If
                ...
                If targetTypes = TargetType.Player Then
                    ...
                    If Not IsInRange(range, GetPetX(index), GetPetY(index), GetPlayerX(target), GetPlayerY(target)) Then
                        Network.PlayerMsg(index, "Target not in range of " & GetPetName(index).Trim & ".", ColorType.Yellow)
                    Else
        ...
        If hasBuffered Then
            ...
            Network.SendActionMsg(mapNum, "Casting " & Skill(skillnum).Name.Trim & "!", ColorType.BrightRed, ActionMsgType.Scroll, GetPetX(index) * 32, GetPetY(index) * 32)
    ...
    Sub SendClearPetSpellBuffer(index As Integer)
        ...
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SClearPetSkillBuffer)
    ...
    Friend Sub PetCastSkill(index As Integer, skillslot As Integer, target As Integer, targetTypes As Byte, Optional takeMana As Boolean = True)
        ...
        If levelReq > Player(index).Character(TempPlayer(index).CurChar).Pet.Level Then
            Network.PlayerMsg(index, GetPetName(index).Trim & " must be level " & levelReq & " to cast this spell.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        ' make sure they have the right access
        If accessReq > GetPlayerAccess(index) Then
            Network.PlayerMsg(index, "You must be an administrator for even your pet to cast this spell.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        Select Case skillCastType
            ...
            Case 1, 3 ' self-cast AOE & targetted AOE
                ...
                ElseIf skillCastType = 3 Then
                    ...
                    If Not IsInRange(range, GetPetX(index), GetPetY(index), x, y) Then
                        Network.PlayerMsg(index, GetPetName(index).Trim & "'s target not in range.", ColorType.Yellow)
                        SendClearPetSpellBuffer(index)
                    End If
                End If
                ...
                Select Case Skill(skillnum).Type
                    ...
                    Case SkillType.DamageHp
                        ...
                        For i = 1 To GetPlayersOnline()
                            If Network.IsPlaying(i) AndAlso i <> index Then
                    ...
                    Case SkillType.HealHp, SkillType.HealMp, SkillType.DamageMp
                        ...
                        For i = 1 To GetPlayersOnline()
                            If Network.IsPlaying(i) AndAlso GetPlayerMap(i) = GetPlayerMap(index) Then
                                If IsInRange(aoE, x, y, GetPlayerX(i), GetPlayerY(i)) Then
            ...
            Case 2 ' targetted
                ...
                If Not IsInRange(range, GetPetX(index), GetPetY(index), x, y) Then
                    Network.PlayerMsg(index, "Target is not in range of your " & GetPetName(index).Trim & "!", ColorType.Yellow)
                    SendClearPetSpellBuffer(index)
                    Exit Sub
                End If
        ...
        If didCast Then
            ...
            Network.SendActionMsg(mapNum, Skill(skillnum).Name.Trim & "!", ColorType.BrightRed, ActionMsgType.Scroll, GetPetX(index) * 32, GetPetY(index) * 32)
        End If
    ...
    Friend Sub SkillPet_Effect(vital As Byte, increment As Boolean, index As Integer, damage As Integer, skillnum As Integer)
        ...
        If damage > 0 Then
            ...
            Network.SendActionMsg(GetPlayerMap(index), sSymbol & damage, colour, ActionMsgType.Scroll, GetPetX(index) * 32, GetPetY(index) * 32)
    ...
    Sub PetAttackPlayer(attacker As Integer, victim As Integer, damage As Integer, Optional skillNum As Integer = 0)
        ...
        If Network.IsPlaying(attacker) = False OrElse Network.IsPlaying(victim) = False OrElse damage < 0 OrElse PetAlive(attacker) = False Then
            Exit Sub
        End If
        ...
        If damage >= GetPlayerVital(victim, VitalType.HP) Then
            Network.SendActionMsg(GetPlayerMap(victim), "-" & GetPlayerVital(victim, VitalType.HP), ColorType.BrightRed, 1, (GetPlayerX(victim) * 32), (GetPlayerY(victim) * 32))

            ' send the sound
            'If SkillNum > 0 Then SendMapSound(Victim, GetPlayerX(Victim), GetPlayerY(Victim), SoundEntity.seSpell, SkillNum)

            ' Player is dead
            Network.GlobalMsg(GetPlayerName(victim) & " has been killed by " & GetPlayerName(attacker) & "'s " & GetPetName(attacker).Trim & ".")
            ...
            For i = 1 To Network.HighIndex
                ...
                If Network.IsPlaying(i) AndAlso Network.IsConnected(i) Then
    ...
    Function CanPetAttackPet(attacker As Integer, victim As Integer, Optional isSkill As Integer = 0) As Boolean
        ...
        ' Check for subscript out of range
        If Not Network.IsPlaying(victim) OrElse Not Network.IsPlaying(attacker) Then Exit Function
        ...
        ' Check to make sure that they dont have access
        If GetPlayerAccess(attacker) > AdminType.Monitor Then
            Network.PlayerMsg(attacker, "Admins cannot attack other players.", ColorType.BrightRed)
            Exit Function
        End If
        ...
        If TempPlayer(attacker).InParty > 0 AndAlso TempPlayer(victim).InParty > 0 Then
            If TempPlayer(attacker).InParty = TempPlayer(victim).InParty Then
                Network.PlayerMsg(attacker, "You can't attack another party member!", ColorType.BrightRed)
                Exit Function
            End If
        End If
    ...
    Sub PetAttackPet(attacker As Integer, victim As Integer, damage As Integer, Optional skillnum As Integer = 0)
        ...
        If Network.IsPlaying(attacker) = False OrElse Network.IsPlaying(victim) = False OrElse damage < 0 OrElse PetAlive(attacker) = False OrElse PetAlive(victim) = False Then
            Exit Sub
        End If
        ...
        If damage >= GetPetVital(victim, VitalType.HP) Then
            ...
            If exp = 0 Then
                Network.PlayerMsg(victim, "You lost no exp.", ColorType.BrightGreen)
                Network.PlayerMsg(attacker, "You received no exp.", ColorType.Yellow)
            Else
                SetPlayerExp(victim, GetPlayerExp(victim) - exp)
                Network.SendExp(victim)
                Network.PlayerMsg(victim, "You lost " & exp & " exp.", ColorType.BrightRed)
            ...
            ' kill pet
            Network.PlayerMsg(victim, "Your " & GetPetName(victim).Trim & " was killed by " & GetPlayerName(attacker).Trim & "'s " & GetPetName(attacker).Trim & "!", ColorType.BrightRed)
            ReleasePet(victim)
        Else
            ' Player not dead, just do the damage
            SetPetVital(victim, VitalType.HP, GetPetVital(victim, VitalType.HP) - damage)
            ...
            Network.SendActionMsg(GetPlayerMap(victim), "-" & damage, ColorType.BrightRed, 1, (GetPetX(victim) * 32), (GetPetY(victim) * 32))
            Network.SendBlood(GetPlayerMap(victim), GetPetX(victim), GetPetY(victim))
    ...
    Friend Sub StunPet(index As Integer, skillnum As Integer)
        ...
        If PetAlive(index) Then
            If Skill(skillnum).StunDuration > 0 Then
                ...
                Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " has been stunned.", ColorType.Yellow)
            End If
        End If
    ...
    Friend Sub HandleHoT_Pet(index As Integer, hotNum As Integer)
        ...
        With TempPlayer(index).PetHoT(hotNum)
            ...
            If .Used AndAlso .Skill > 0 Then
                ' time to tick?
                If GetTimeMs() > .Timer + (Skill(.Skill).Interval * 1000) Then
                    Network.SendActionMsg(GetPlayerMap(index), "+" & Skill(.Skill).Vital, ColorType.BrightGreen, ActionMsgType.Scroll, Player(index).Character(TempPlayer(index).CurChar).Pet.X * 32, Player(index).Character(TempPlayer(index).CurChar).Pet.Y * 32,)
    ...
    Friend Sub TryPetAttackPlayer(index As Integer, victim As Integer)
        ...
        If CanPetAttackPlayer(index, victim) Then
            ...
            If CanPetCrit(index) Then
                damage = damage * 1.5
                Network.SendActionMsg(mapNum, "Critical!", ColorType.BrightCyan, 1, (GetPetX(index) * 32), (GetPetY(index) * 32))
            End If
    ...
    Friend Sub TryPetAttackPet(index As Integer, victim As Integer)
        ...
        If CanPetAttackPet(index, victim) Then
            ...
            If CanPetDodge(victim) Then
                Network.SendActionMsg(mapNum, "Dodge!", ColorType.Pink, 1, (GetPetX(victim) * 32), (GetPetY(victim) * 32))
                Exit Sub
            End If

            If CanPetParry(victim) Then
                Network.SendActionMsg(mapNum, "Parry!", ColorType.Pink, 1, (GetPetX(victim) * 32), (GetPetY(victim) * 32))
                Exit Sub
            End If
    ...
    Function CanPlayerAttackPet(attacker As Integer, victim As Integer, Optional isSkill As Boolean = False) As Boolean
        ...
        If Not Map(GetPlayerMap(attacker)).Moral = MapMoralType.None Then
            If GetPlayerPK(victim) = 0 Then
                Network.PlayerMsg(attacker, "This is a safe zone!", ColorType.Yellow)
                Exit Function
            End If
        End If
        ...
        ' Check to make sure the victim isn't an admin
        If GetPlayerAccess(victim) > AdminType.Monitor Then
            Network.PlayerMsg(attacker, "You cannot attack " & GetPlayerName(victim) & "s " & GetPetName(victim).Trim & "!", ColorType.BrightRed)
            Exit Function
        End If
    ...
    Sub PlayerAttackPet(attacker As Integer, victim As Integer, damage As Integer, Optional skillnum As Integer = 0)
        ...
        If Network.IsPlaying(attacker) = False OrElse Network.IsPlaying(victim) = False OrElse damage < 0 OrElse Not PetAlive(victim) Then Exit Sub
        ' Check for weapon
        n = 0
        ...
        If damage >= GetPetVital(victim, VitalType.HP) Then
            Network.SendActionMsg(GetPlayerMap(victim), "-" & GetPetVital(victim, VitalType.HP), ColorType.BrightRed, 1, (GetPetX(victim) * 32), (GetPetY(victim) * 32))
            ...
            ' purge target info of anyone who targetted dead guy
            For i = 1 To GetPlayersOnline()
                If Network.IsPlaying(i) AndAlso Network.IsConnected(i) AndAlso GetPlayerMap(i) = GetPlayerMap(attacker) Then
            ...
            Network.PlayerMsg(victim, ("Your " & GetPetName(victim).Trim & " was killed by  " & GetPlayerName(attacker).Trim & "."), ColorType.BrightRed)
            ReCallPet(victim)
        Else
            ' Pet not dead, just do the damage
            SetPetVital(victim, VitalType.HP, GetPetVital(victim, VitalType.HP) - damage)
    ...
    Sub CheckPetLevelUp(index As Integer)
        ...
        If levelCount > 0 Then
            If levelCount = 1 Then
                'singular
                Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " has gained " & levelCount & " level!", ColorType.BrightGreen)
            Else
                'plural
                Network.PlayerMsg(index, "Your " & GetPetName(index).Trim & " has gained " & levelCount & " levels!", ColorType.BrightGreen)
            End If
