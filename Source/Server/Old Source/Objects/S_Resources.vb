...
#Region "DataBase"
    ...
    Sub SaveResource(ResourceNum As Integer)
        ...
        filename = Path.Combine(Environment.CurrentDirectory, "data", "resources", String.Format("resource{0}.dat", ResourceNum))
    ...
    Sub CheckResources()
        For i = 1 To MAX_RESOURCES
            ...
            If Not File.Exists(Path.Combine(Environment.CurrentDirectory, "data", "resources", String.Format("resource{0}.dat", i))) Then
                SaveResource(i)
            End If
    ...
    Function ResourcesData() As Byte()
        ...
        For i = 1 To MAX_RESOURCES
            If Not Resource(i).Name.Trim.Length > 0 Then Continue For
    ...
    Sub LoadSkillExp()
        ...
        Dim path = Environment.CurrentDirectory & "\Data\SkillExp.ini"
    ...
    Sub CheckResourceLevelUp(index As Integer, SkillSlot As Integer)
        ...
        If level_count > 0 Then
            If level_count = 1 Then
                'singular
                Network.PlayerMsg(index, String.Format("Your {0} has gone up a level!", GetResourceSkillName(SkillSlot)), ColorType.BrightGreen)
            Else
                'plural
                Network.PlayerMsg(index, String.Format("Your {0} has gone up by {1} levels!", GetResourceSkillName(SkillSlot), level_count), ColorType.BrightGreen)
            End If
...
#Region "Incoming Packets"
    ...
    Sub Packet_EditResource(index As Integer, ByRef data() As Byte)
        ...
        Buffer.WriteInt32(ServerPacket.SResourceEditor)
...
#Region "Outgoing Packets"
    ...
    Sub SendResourceCacheTo(index As Integer, Resource_num As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SResourceCache)
    ...
    Sub SendResources(index As Integer)
        ...
        For i = 1 To MAX_RESOURCES
            ...
            If Resource(i).Name.Trim.Length > 0 Then
    ...
    Sub SendUpdateResourceTo(index As Integer, ResourceNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SUpdateResource)
                    ...
                    If Item(GetPlayerEquipment(index, EquipmentType.Weapon)).Data3 = Resource(Resource_index).ToolRequired Then
                        ...
                        If Resource(Resource_index).ItemReward > 0 Then
                            If FindOpenInvSlot(index, Resource(Resource_index).ItemReward) = 0 Then
                                Network.PlayerMsg(index, "You have no inventory space.", ColorType.Yellow)
                                Exit Sub
                            End If
                        End If
                        ...
                        If Resource(Resource_index).LvlRequired > GetPlayerGatherSkillLvl(index, ResourceType) Then
                            Network.PlayerMsg(index, "Your level is too low!", ColorType.Yellow)
                            Exit Sub
                        End If
                        ...
                        If ResourceCache(GetPlayerMap(index)).ResourceData(Resource_num).ResourceState = 0 Then
                            ...
                            If Damage > 0 Then
                                ...
                                If ResourceCache(GetPlayerMap(index)).ResourceData(Resource_num).CurHealth - Damage <= 0 Then
                                    ...
                                    Network.SendActionMsg(GetPlayerMap(index), Resource(Resource_index).SuccessMessage.Trim, ColorType.BrightGreen, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32))
                                    ...
                                    'send msg
                                    Network.PlayerMsg(index, String.Format("Your {0} has earned {1} experience. ({2}/{3})", GetResourceSkillName(ResourceType), Resource(Resource_index).ExpReward, GetPlayerGatherSkillExp(index, ResourceType), GetPlayerGatherSkillMaxExp(index, ResourceType)), ColorType.BrightGreen)
                                    Network.SendPlayerData(index)
                                ...
                                Else
                                    ...
                                    Network.SendActionMsg(GetPlayerMap(index), "-" & Damage, ColorType.BrightRed, 1, (rX * 32), (rY * 32))
                                    SendAnimation(GetPlayerMap(index), Resource(Resource_index).Animation, rX, rY)
                                End If
                                CheckTasks(index, QuestType.Gather, Resource_index)
                            Else
                                ' too weak
                                Network.SendActionMsg(GetPlayerMap(index), "Miss!", ColorType.BrightRed, 1, (rX * 32), (rY * 32))
                            End If
                        Else
                            Network.SendActionMsg(GetPlayerMap(index), Resource(Resource_index).EmptyMessage.Trim, ColorType.BrightRed, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32))
                        End If
                    Else
                        Network.PlayerMsg(index, "You have the wrong type of tool equiped.", ColorType.Yellow)
                    End If
                Else
                    Network.PlayerMsg(index, "You need a tool to gather this resource.", ColorType.Yellow)
                End If
            End If
        End If
    End Sub
