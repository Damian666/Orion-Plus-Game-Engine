...
#Region "PlayerCombat"
    ...
    Function CanPlayerAttackPlayer(Attacker As Integer, Victim As Integer, Optional IsSkill As Boolean = False) As Boolean
        ...
        If Not Network.IsPlaying(Victim) Then Exit Function
        ...
        If Not Map(GetPlayerMap(Attacker)).Moral = MapMoralType.None Then
            If GetPlayerPK(Victim) = False Then
                Network.PlayerMsg(Attacker, "This is a safe zone!", ColorType.BrightRed)
                Exit Function
            End If
        End If
        ...
        If GetPlayerAccess(Attacker) > AdminType.Monitor Then
            Network.PlayerMsg(Attacker, "You cannot attack any player for thou art an admin!", ColorType.BrightRed)
            Exit Function
        End If

        ' Check to make sure the victim isn't an admin
        If GetPlayerAccess(Victim) > AdminType.Monitor Then
            Network.PlayerMsg(Attacker, "You cannot attack " & GetPlayerName(Victim) & "!", ColorType.BrightRed)
            Exit Function
        End If

        ' Make sure attacker is high enough level
        If GetPlayerLevel(Attacker) < 10 Then
            Network.PlayerMsg(Attacker, "You are below level 10, you cannot attack another player yet!", ColorType.BrightRed)
            Exit Function
        End If

        ' Make sure victim is high enough level
        If GetPlayerLevel(Victim) < 10 Then
            Network.PlayerMsg(Attacker, GetPlayerName(Victim) & " is below level 10, you cannot attack this player yet!", ColorType.BrightRed)
            Exit Function
        End If
    ...
    Function GetPlayerDamage(index As Integer) As Integer
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse index <= 0 OrElse index > MAX_PLAYERS Then
            Exit Function
        End If
    ...
    Sub AttackPlayer(Attacker As Integer, Victim As Integer, Damage As Integer, Optional skillnum As Integer = 0, Optional npcnum As Integer = 0)
        ...
        If npcnum = 0 Then
            ' Check for subscript out of range
            If Network.IsPlaying(Attacker) = False OrElse Network.IsPlaying(Victim) = False OrElse Damage < 0 Then
                Exit Sub
            End If
            ...
            buffer.WriteInt32(ServerPacket.SAttack)
            buffer.WriteInt32(Attacker)
            Network.SendDataToMapBut(Attacker, GetPlayerMap(Attacker), buffer.ToPacket)
            buffer.Dispose()

            If Damage >= GetPlayerVital(Victim, VitalType.HP) Then

                Network.SendActionMsg(GetPlayerMap(Victim), "-" & Damage, ColorType.BrightRed, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))

                ' Player is dead
                Network.GlobalMsg(GetPlayerName(Victim) & " has been killed by " & GetPlayerName(Attacker))
                ...
                If exp = 0 Then
                    Network.PlayerMsg(Victim, "You lost no exp.", ColorType.BrightGreen)
                    Network.PlayerMsg(Attacker, "You received no exp.", ColorType.BrightRed)
                Else
                    SetPlayerExp(Victim, GetPlayerExp(Victim) - exp)
                    Network.SendExp(Victim)
                    Network.PlayerMsg(Victim, "You lost " & exp & " exp.", ColorType.BrightRed)
                    SetPlayerExp(Attacker, GetPlayerExp(Attacker) + exp)
                    Network.SendExp(Attacker)
                    Network.PlayerMsg(Attacker, "You received " & exp & " exp.", ColorType.BrightGreen)
                End If
                ...
                If GetPlayerPK(Victim) = False Then
                    If GetPlayerPK(Attacker) = False Then
                        ...
                        Network.SendPlayerData(Attacker)
                        Network.GlobalMsg(GetPlayerName(Attacker) & " has been deemed a Player Killer!!!")
                    End If
                Else
                    Network.GlobalMsg(GetPlayerName(Victim) & " has paid the price for being a Player Killer!!!")
                End If
            ...
            Else
                ...
                Network.SendVital(Victim, VitalType.HP)
                Network.SendActionMsg(GetPlayerMap(Victim), "-" & Damage, ColorType.BrightRed, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))
        ...
        Else ' npc to player
            ' Check for subscript out of range
            If Network.IsPlaying(Victim) = False OrElse Damage < 0 Then Exit Sub

            mapNum = GetPlayerMap(Victim)

            ' Send this packet so they can see the person attacking
            buffer = New ByteStream(4)
            buffer.WriteInt32(ServerPacket.SNpcAttack)
            buffer.WriteInt32(Attacker)
            ...
            If Damage >= GetPlayerVital(Victim, VitalType.HP) Then
                ...
                Network.SendActionMsg(mapNum, "-" & Damage, ColorType.BrightRed, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))

                ' Player is dead
                Network.GlobalMsg(GetPlayerName(Victim) & " has been killed by " & Npc(MapNpc(mapNum).Npc(Attacker).Num).Name)
    ...
    Friend Sub StunPlayer(index As Integer, skillnum As Integer)
        ...
        If Skill(skillnum).StunDuration > 0 Then
            ...
            Network.SendStunned(index)
            ' tell him he's stunned
            Network.PlayerMsg(index, "You have been stunned!", ColorType.Yellow)
        End If
    End Sub
    ...
    Function CanPlayerAttackNpc(Attacker As Integer, MapNpcNum As Integer, Optional IsSkill As Boolean = False) As Boolean
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(Attacker) = False OrElse MapNpcNum <= 0 OrElse MapNpcNum > MAX_MAP_NPCS Then
            Exit Function
        End If
        ...
        ' Make sure they are on the same map
        If Network.IsPlaying(Attacker) Then
                        ...
                        Else
                            If Npc(NpcNum).Behaviour = NpcBehavior.Quest Then
                                If QuestCompleted(Attacker, Npc(NpcNum).QuestNum) Then
                                    Network.PlayerMsg(Attacker, Npc(NpcNum).Name.Trim & ": " & Npc(NpcNum).AttackSay.Trim, ColorType.Yellow)
                                    Exit Function
    ...
    Sub PlayerAttackNpc(Attacker As Integer, MapNpcNum As Integer, Damage As Integer)
        ' Check for subscript out of range
        If Network.IsPlaying(Attacker) = False OrElse MapNpcNum <= 0 OrElse MapNpcNum > MAX_MAP_NPCS OrElse Damage < 0 Then Exit Sub
        ...
        Network.SendActionMsg(MapNum, "-" & Damage, ColorType.BrightRed, 1, (MapNpc(MapNum).Npc(MapNpcNum).X * 32), (MapNpc(MapNum).Npc(MapNpcNum).Y * 32))
        Network.SendBlood(GetPlayerMap(Attacker), MapNpc(MapNum).Npc(MapNpcNum).X, MapNpc(MapNum).Npc(MapNpcNum).Y)
        Network.SendPlayerAttack(Attacker)
        If Weapon > 0 Then
        ...
        If Not IsNpcDead(MapNum, MapNpcNum) Then
            ...
            If MapNpc(MapNum).Npc(MapNpcNum).Target = 0 Then
                If Npc(NpcNum).AttackSay.Trim.Length > 0 Then
                    Network.PlayerMsg(Attacker, String.Format("{0} says: '{1}'", Npc(NpcNum).Name.Trim, Npc(NpcNum).AttackSay.Trim), ColorType.Yellow)
                End If
            End If
    ...
    Friend Sub SpellPlayer_Effect(Vital As Byte, increment As Boolean, index As Integer, Damage As Integer, Skillnum As Integer)
        ...
        If Damage > 0 Then
            ...
            Network.SendActionMsg(GetPlayerMap(index), sSymbol & Damage, Colour, ActionMsgType.Scroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32)
    ...
    Friend Sub TryPlayerAttackPlayer(Attacker As Integer, Victim As Integer)
        ...
        If CanPlayerAttackPlayer(Attacker, Victim) Then
            ...
            If CanPlayerDodge(Victim) Then
                Network.SendActionMsg(mapNum, "Dodge!", ColorType.Pink, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))
                Exit Sub
            End If

            If CanPlayerParry(Victim) Then
                Network.SendActionMsg(mapNum, "Parry!", ColorType.Pink, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))
                Exit Sub
            End If
            ...
            If CanPlayerBlockHit(Victim) Then
                Network.SendActionMsg(mapNum, "Block!", ColorType.BrightCyan, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))
                Damage = 0
                Exit Sub
            Else
                ...
                If CanPlayerCriticalHit(Attacker) Then
                    Damage = Damage * 1.5
                    Network.SendActionMsg(mapNum, "Critical!", ColorType.BrightCyan, 1, (GetPlayerX(Attacker) * 32), (GetPlayerY(Attacker) * 32))
                End If
            End If
            ...
            Else
                Network.PlayerMsg(Attacker, "Your attack does nothing.", ColorType.BrightRed)
            End If
    ...
    Sub PlayerAttackPlayer(Attacker As Integer, Victim As Integer, Damage As Integer)
        ...
        End If
        Network.SendActionMsg(GetPlayerMap(Victim), "-" & Damage, ColorType.BrightRed, 1, (GetPlayerX(Victim) * 32), (GetPlayerY(Victim) * 32))
        Network.SendBlood(GetPlayerMap(Victim), GetPlayerX(Victim), GetPlayerY(Victim))
    ...
    Friend Sub TryPlayerAttackNpc(index As Integer, mapnpcnum As Integer)
        ...
        If CanPlayerAttackNpc(index, mapnpcnum) Then
            ...
            If CanNpcDodge(npcnum) Then
                Network.SendActionMsg(mapNum, "Dodge!", ColorType.Pink, 1, (MapNpc(mapNum).Npc(mapnpcnum).X * 32), (MapNpc(mapNum).Npc(mapnpcnum).Y * 32))
                Exit Sub
            End If

            If CanNpcParry(npcnum) Then
                Network.SendActionMsg(mapNum, "Parry!", ColorType.Pink, 1, (MapNpc(mapNum).Npc(mapnpcnum).X * 32), (MapNpc(mapNum).Npc(mapnpcnum).Y * 32))
                Exit Sub
            End If
            ...
            If CanNpcBlock(npcnum) Then
                Network.SendActionMsg(mapNum, "Block!", ColorType.BrightCyan, 1, (MapNpc(mapNum).Npc(mapnpcnum).X * 32), (MapNpc(mapNum).Npc(mapnpcnum).Y * 32))
                Damage = 0
                Exit Sub
            Else
                ...
                If CanPlayerCriticalHit(index) Then
                    Damage = Damage * 1.5
                    Network.SendActionMsg(mapNum, "Critical!", ColorType.BrightCyan, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32))
                End If
            ...
            Network.SendTarget(index, mapnpcnum, TargetType.Npc)

            If Damage > 0 Then
                PlayerAttackNpc(index, mapnpcnum, Damage)
            Else
                Network.PlayerMsg(index, "Your attack does nothing.", ColorType.BrightRed)
            End If
    ...
    Friend Sub HandlePlayerKillPlayer(Attacker As Integer, Victim As Integer)
        ...
        Network.GlobalMsg(String.Format("{0} has been killed by {1}!", GetPlayerName(Victim), GetPlayerName(Attacker)))
        ...
        For Each p In TempPlayer.Where(Function(x, i) x.InGame AndAlso GetPlayerMap(i + 1) = GetPlayerMap(Victim) AndAlso x.TargetType = TargetType.Player AndAlso x.Target = Victim).Select(Function(x, i) i + 1).ToArray()
            ...
            Network.SendTarget(p, 0, TargetType.None)
        Next
    ...
    Friend Sub HandlePlayerKillNpc(mapNum As Integer, index As Integer, MapNpcNum As Integer)
        ...
        Network.SendTarget(index, 0, TargetType.None)
                ...
                For x = 1 To EquipmentType.Count - 1
                    If GetPlayerEquipment(Victim, x) > 0 Then
                        ...
                        If j = z Then
                            ...
                            Network.PlayerMsg(Victim, "In death you lost grip on your " & Item(GetPlayerEquipment(Victim, x)).Name.Trim, ColorType.BrightRed)
                            ...
                            Network.SendWornEquipment(Victim)
                            Network.SendMapEquipment(Victim)
                        End If
                    End If
                Next
            Else
                ...
                For x = 1 To MAX_INV
                    If GetPlayerInvItemNum(Victim, x) > 0 Then
                        ...
                        If j = z Then
                            ...
                            Network.PlayerMsg(Victim, "In death you lost grip on your " & Item(GetPlayerInvItemNum(Victim, x)).Name.Trim, ColorType.BrightRed)
...
#Region "Data"
    ...
    Function GetPlayerLogin(index As Integer) As String
        GetPlayerLogin = Player(index).Login.Trim
    End Function
    ...
    Sub CheckPlayerLevelUp(index As Integer)
        ...
        If level_count > 0 Then
            If level_count = 1 Then
                'singular
                Network.GlobalMsg(GetPlayerName(index) & " has gained " & level_count & " level!")
            Else
                'plural
                Network.GlobalMsg(GetPlayerName(index) & " has gained " & level_count & " levels!")
            End If
            Network.SendExp(index)
            Network.SendPlayerData(index)
        End If
    End Sub
...
#Region "Incoming Packets"
    ...
    Friend Sub HandleUseChar(index As Integer)
        If Not Network.IsPlaying(index) Then
            JoinGame(index)
            Dim text = String.Format("{0} | {1} has began playing {2}.", GetPlayerLogin(index), GetPlayerName(index), Configuration.Settings.GameName)
...
#Region "Outgoing Packets"
    ...
    Sub SendLeaveMap(index As Integer, mapNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SLeftMap)
        buffer.WriteInt32(index)
        Network.SendDataToMapBut(index, mapNum, buffer.ToPacket)
...
#Region "Movement"
    ...
    Sub PlayerWarp(index As Integer, MapNum As Integer, X As Integer, Y As Integer, Optional HouseTeleport As Boolean = False, Optional NoInstance As Boolean = False)
        ...
        If Map(MapNum).Instanced = 1 And NoInstance = False Then
            ...
            If MapNum = -1 Then
                ...
                X = GetPlayerX(index)
                Y = GetPlayerY(index)
                Network.AlertMsg(index, "Unable to create a cached map!")
            Else
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse MapNum <= 0 OrElse MapNum > MAX_CACHED_MAPS Then Exit Sub
        ...
        If Player(index).Character(TempPlayer(index).CurChar).InHouse > 0 Then
            If Network.IsPlaying(Player(index).Character(TempPlayer(index).CurChar).InHouse) Then
            ...
            For i = 1 To GetPlayersOnline()
                If Network.IsPlaying(i) Then
                    If GetPlayerMap(i) = MapNum Then
                        Network.SendMapEquipmentTo(i, index)
                    End If
                End If
            Next
        End If

        ' Now we check if there were any players left on the map the player just left, and if not
        ' stop processing npcs
        ...
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SCheckForMap)
    ...
    Sub PlayerMove(index As Integer, Dir As Integer, Movement As Integer, ExpectingWarp As Boolean)
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse Dir < DirectionType.Up OrElse Dir > DirectionType.Right OrElse Movement < 1 OrElse Movement > 2 Then
            Exit Sub
        End If
                        ...
                        If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type <> TileType.Blocked Then
                            If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type <> TileType.Resource Then
                                ...
                                If Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type <> TileType.Key OrElse (Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index) - 1).Type = TileType.Key AndAlso TempTile(GetPlayerMap(index)).DoorOpen(GetPlayerX(index), GetPlayerY(index) - 1) = True) Then
                                    ...
                                    Network.SendPlayerMove(index, Movement)
                                    Moved = True
                                End If
        ...
        With Map(GetPlayerMap(index)).Tile(GetPlayerX(index), GetPlayerY(index))
            ...
            If .Type = TileType.Door Then
                ...
                Network.SendDoorAnimation(GetPlayerMap(index), GetPlayerX(index), GetPlayerY(index))
            ...
            If .Type = TileType.KeyOpen Then
                ...
                If Map(GetPlayerMap(index)).Tile(x, y).Type = TileType.Key AndAlso TempTile(GetPlayerMap(index)).DoorOpen(x, y) = False Then
                    ...
                    Network.SendMapKey(index, x, y, 1)
                    Network.MapMsg(GetPlayerMap(index), "A door has been unlocked.", ColorType.White)
                End If
            End If
            ...
            If .Type = TileType.Shop Then
                ...
                If x > 0 Then ' shop exists?
                    If Shop(x).Name.Trim.Length > 0 Then ' name exists?
                        Network.SendOpenShop(index, x)
            ...
            If .Type = TileType.Heal Then
                ...
                If Not GetPlayerVital(index, VitalType) = GetPlayerMaxVital(index, VitalType) Then
                    ...
                    End If
                    Network.SendActionMsg(GetPlayerMap(index), "+" & amount, Colour, ActionMsgType.Scroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32, 1)
                    SetPlayerVital(index, VitalType, GetPlayerVital(index, VitalType) + amount)
                    Network.PlayerMsg(index, "You feel rejuvinating forces coarsing through your body.", ColorType.BrightGreen)
                    Network.SendVital(index, VitalType)
            ...
            If .Type = TileType.Trap Then
                amount = .Data1
                Network.SendActionMsg(GetPlayerMap(index), "-" & amount, ColorType.BrightRed, ActionMsgType.Scroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32, 1)
                If GetPlayerVital(index, Enums.VitalType.HP) - amount <= 0 Then
                    KillPlayer(index)
                    Network.PlayerMsg(index, "You've been killed by a trap.", ColorType.BrightRed)
                Else
                    SetPlayerVital(index, Enums.VitalType.HP, GetPlayerVital(index, Enums.VitalType.HP) - amount)
                    Network.PlayerMsg(index, "You've been injured by a trap.", ColorType.BrightRed)
                    Network.SendVital(index, Enums.VitalType.HP)
                    ' send vitals to party if in one
                    If TempPlayer(index).InParty > 0 Then SendPartyVitals(TempPlayer(index).InParty, index)
                End If
                Moved = True
            ...
            If .Type = TileType.House Then
                If Player(index).Character(TempPlayer(index).CurChar).House.Houseindex = .Data1 Then
                    ...
                    Dim data = Network.PlayerData(index)
                    Network.SendDataTo(index, data)
                ...
                Else
                    ...
                    Buffer = New ByteStream(4)
                    Buffer.WriteInt32(ServerPacket.SBuyHouse)
...
#Region "Inventory"
    ...
    Function HasItem(index As Integer, ItemNum As Integer) As Integer
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse ItemNum <= 0 OrElse ItemNum > MAX_ITEMS Then
            Exit Function
        End If
    ...
    Sub PlayerMapGetItem(index As Integer)
        ...
        If Not Network.IsPlaying(index) Then Exit Sub
        mapNum = GetPlayerMap(index)
                        ...
                        If (MapItem(mapNum, i).Y = GetPlayerY(index)) Then
                            ...
                            If n <> 0 Then
                                ...
                                If Item(itemnum).Randomize <> 0 Then
                                    If MapItem(mapNum, i).RandData.Prefix.Trim <> "" OrElse MapItem(mapNum, i).RandData.Suffix.Trim <> "" Then
                                ...
                                If Item(GetPlayerInvItemNum(index, n)).Type = ItemType.Currency OrElse Item(GetPlayerInvItemNum(index, n)).Stackable = 1 Then
                                    ...
                                    Msg = MapItem(mapNum, i).Value & " " & Item(GetPlayerInvItemNum(index, n)).Name.Trim
                                Else
                                    SetPlayerInvItemValue(index, n, 0)
                                    Msg = CheckGrammar(Item(GetPlayerInvItemNum(index, n)).Name.Trim, 1)
                                End If
                                ...
                                Network.SendInventoryUpdate(index, n)
                                SpawnItemSlot(i, 0, 0, GetPlayerMap(index), 0, 0)

                                Network.SendActionMsg(GetPlayerMap(index), Msg, ColorType.White, 1, (GetPlayerX(index) * 32), (GetPlayerY(index) * 32))
                                CheckTasks(index, QuestType.Gather, GetItemNum(Item(GetPlayerInvItemNum(index, n)).Name.Trim))
                                Exit For
                            Else
                                Network.PlayerMsg(index, "Your inventory is full.", ColorType.BrightRed)
                                Exit For
                            End If
                        End If
                    End If
    ...
    Function FindOpenInvSlot(index As Integer, ItemNum As Integer) As Integer
        ...
        If Item(ItemNum).Type = ItemType.Currency OrElse Item(ItemNum).Stackable = 1 Then
            ' If currency then check to see if they already have an instance of the item and add it
            ' to that
            For i = 1 To MAX_INV
                If GetPlayerInvItemNum(index, i) = ItemNum Then
                    FindOpenInvSlot = i
        ...
        For i = 1 To MAX_INV
            ...
            If GetPlayerInvItemNum(index, i) = ItemNum Then
                If Item(ItemNum).Type = ItemType.Currency OrElse Item(ItemNum).Stackable = 1 Then
                    ...
                    ' Is what we are trying to take away more then what they have? If so just set it
                    ' to zero
                    If ItemVal >= GetPlayerInvItemValue(index, i) Then
                        TakeInvItem = True
                    Else
                        SetPlayerInvItemValue(index, i, GetPlayerInvItemValue(index, i) - ItemVal)
                        Network.SendInventoryUpdate(index, i)
                    End If
                Else
                    TakeInvItem = True
                End If
    ...
    Function GiveInvItem(index As Integer, ItemNum As Integer, ItemVal As Integer, Optional SendUpdate As Boolean = True) As Boolean
        ...
        If i <> 0 Then
            ...
            SetPlayerInvItemValue(index, i, GetPlayerInvItemValue(index, i) + ItemVal)
            If SendUpdate Then Network.SendInventoryUpdate(index, i)
            GiveInvItem = True
        Else
            Network.PlayerMsg(index, "Your inventory is full.", ColorType.BrightRed)
            GiveInvItem = False
    ...
    Sub PlayerMapDropItem(index As Integer, InvNum As Integer, Amount As Integer)
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse InvNum <= 0 OrElse InvNum > MAX_INV Then
            Exit Sub
        End If
            ...
            If (GetPlayerInvItemNum(index, InvNum) <= MAX_ITEMS) Then
                ...
                If i <> 0 Then
                    ...
                    If Item(GetPlayerInvItemNum(index, InvNum)).Type = ItemType.Currency OrElse Item(GetPlayerInvItemNum(index, InvNum)).Stackable = 1 Then
                        ...
                        End If
                        Call Network.MapMsg(GetPlayerMap(index), String.Format("{0} has dropped {1} ({2}x).", GetPlayerName(index), CheckGrammar(Item(GetPlayerInvItemNum(index, InvNum)).Name.Trim), Amount), ColorType.Yellow)
                    Else
                        ' Its not a currency object so this is easy
                        MapItem(GetPlayerMap(index), i).Value = 0
                        ' send message

                        Network.MapMsg(GetPlayerMap(index), String.Format("{0} has dropped {1}.", GetPlayerName(index), CheckGrammar(Item(GetPlayerInvItemNum(index, InvNum)).Name.Trim)), ColorType.Yellow)
                        SetPlayerInvItemNum(index, InvNum, 0)
                        SetPlayerInvItemValue(index, InvNum, 0)
                    End If

                    ' Send inventory update
                    Network.SendInventoryUpdate(index, InvNum)
                ...
                Else
                    Network.PlayerMsg(index, "Too many items already on the ground.", ColorType.Yellow)
                End If
            End If
        End If
    ...
    Function TakeInvSlot(index As Integer, InvSlot As Integer, ItemVal As Integer) As Boolean
        ...
        ' Check for subscript out of range
        If Network.IsPlaying(index) = False OrElse InvSlot <= 0 OrElse InvSlot > MAX_ITEMS Then Exit Function
        ...
        If (GetPlayerInvItemNum(index, InvNum) > 0) AndAlso (GetPlayerInvItemNum(index, InvNum) <= MAX_ITEMS) Then
            ...
            Select Case Item(InvItemNum).Type
                Case ItemType.Equipment
                    For i = 1 To StatType.Count - 1
                        If GetPlayerStat(index, i) < Item(InvItemNum).Stat_Req(i) Then
                            Network.PlayerMsg(index, "You do not meet the stat requirements to equip this item.", ColorType.BrightRed)
                            Exit Sub
                        End If
                    Next
                    ...
                    If i > GetPlayerLevel(index) Then
                        Network.PlayerMsg(index, "You do not meet the level requirements to equip this item.", ColorType.BrightRed)
                        Exit Sub
                    End If

                    ' Make sure they are the right class
                    If Not Item(InvItemNum).ClassReq = GetPlayerClass(index) AndAlso Not Item(InvItemNum).ClassReq = 0 Then
                        Network.PlayerMsg(index, "You do not meet the class requirements to equip this item.", ColorType.BrightRed)
                        Exit Sub
                    End If

                    ' access requirement
                    If Not GetPlayerAccess(index) >= Item(InvItemNum).AccessReq Then
                        Network.PlayerMsg(index, "You do not meet the access requirement to equip this item.", ColorType.BrightRed)
                        Exit Sub
                    End If
                    ...
                    Select Case Item(InvItemNum).SubType
                        Case EquipmentType.Weapon
                            ...
                            If Item(InvItemNum).TwoHanded > 0 Then
                                If GetPlayerEquipment(index, EquipmentType.Shield) > 0 Then
                                    Network.PlayerMsg(index, "This is a 2Handed weapon! Please unequip shield first.", ColorType.BrightRed)
                                    Exit Sub
                                End If
                            End If
                            ...
                            If Item(InvItemNum).Randomize <> 0 Then
                                Network.PlayerMsg(index, "You equip " & tempstr(1) & " " & CheckGrammar(Item(InvItemNum).Name) & " " & tempstr(2), ColorType.BrightGreen)
                            Else
                                Network.PlayerMsg(index, "You equip " & CheckGrammar(Item(InvItemNum).Name), ColorType.BrightGreen)
                            End If
                            ...
                            Network.SendWornEquipment(index)
                            Network.SendMapEquipment(index)
                            Network.SendInventory(index)
                            Network.SendInventoryUpdate(index, InvNum)
                            Network.SendStats(index)

                            ' send vitals
                            Network.SendVitals(index)
                        ...
                        Case EquipmentType.Shield
                            If Item(GetPlayerEquipment(index, EquipmentType.Weapon)).TwoHanded > 0 Then
                                Network.PlayerMsg(index, "Please unequip your 2handed weapon first.", ColorType.BrightRed)
                                Exit Sub
                            End If
                ...
                Case ItemType.Consumable
                    ...
                    If i > GetPlayerLevel(index) Then
                        Network.PlayerMsg(index, "You do not meet the level requirements to use this item.", ColorType.BrightRed)
                        Exit Sub
                    End If

                    ' Make sure they are the right class
                    If Not Item(InvItemNum).ClassReq = GetPlayerClass(index) AndAlso Not Item(InvItemNum).ClassReq = 0 Then
                        Network.PlayerMsg(index, "You do not meet the class requirements to use this item.", ColorType.BrightRed)
                        Exit Sub
                    End If
                    ...
                    Select Case Item(InvItemNum).SubType
                        Case ConsumableType.Hp
                            Network.SendActionMsg(GetPlayerMap(index), "+" & Item(InvItemNum).Data1, ColorType.BrightGreen, ActionMsgType.Scroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32)
                            ...
                            Else
                                TakeInvItem(index, InvItemNum, 0)
                            End If
                            Network.SendVital(index, VitalType.HP)

                            ' send vitals to party if in one
                            If TempPlayer(index).InParty > 0 Then SendPartyVitals(TempPlayer(index).InParty, index)

                        Case ConsumableType.Mp
                            Network.SendActionMsg(GetPlayerMap(index), "+" & Item(InvItemNum).Data1, ColorType.BrightBlue, ActionMsgType.Scroll, GetPlayerX(index) * 32, GetPlayerY(index) * 32)
                            SendAnimation(GetPlayerMap(index), Item(InvItemNum).Animation, 0, 0, TargetType.Player, index)
                            ...
                            Else
                                TakeInvItem(index, InvItemNum, 0)
                            End If
                            Network.SendVital(index, VitalType.MP)
                        ...
                        Case ConsumableType.Mp
                            ...
                            Else
                                TakeInvItem(index, InvItemNum, 0)
                            End If
                            Network.SendVital(index, VitalType.SP)
                ...
                Case ItemType.Key
                    ...
                    If Map(GetPlayerMap(index)).Tile(x, y).Type = TileType.Key Then
                        ...
                        If InvItemNum = Map(GetPlayerMap(index)).Tile(x, y).Data1 Then
                            ...
                            TempTile(GetPlayerMap(index)).DoorTimer = GetTimeMs()
                            Network.SendMapKey(index, x, y, 1)
                            Network.MapMsg(GetPlayerMap(index), "A door has been unlocked.", ColorType.Yellow)
                            ...
                            If Map(GetPlayerMap(index)).Tile(x, y).Data2 = 1 Then
                                TakeInvItem(index, InvItemNum, 0)
                                Network.PlayerMsg(index, "The key is destroyed in the lock.", ColorType.Yellow)
                            End If
                        End If
                    End If
                            ...
                            If i <= GetPlayerLevel(index) Then
                                ...
                                If i > 0 Then
                                    ...
                                    If Not HasSkill(index, n) Then
                                        ...
                                        SendAnimation(GetPlayerMap(index), Item(InvItemNum).Animation, 0, 0, TargetType.Player, index)
                                        TakeInvItem(index, InvItemNum, 0)
                                        Network.PlayerMsg(index, "You study the skill carefully.", ColorType.Yellow)
                                        Network.PlayerMsg(index, "You have learned a new skill!", ColorType.BrightGreen)
                                    Else
                                        Network.PlayerMsg(index, "You have already learned this skill!", ColorType.BrightRed)
                                    End If
                                Else
                                    Network.PlayerMsg(index, "You have learned all that you can learn!", ColorType.BrightRed)
                                End If
                            Else
                                Network.PlayerMsg(index, "You must be level " & i & " to learn this skill.", ColorType.Yellow)
                            End If
                        Else
                            Network.PlayerMsg(index, "This skill can only be learned by " & CheckGrammar(GetClassName(Skill(n).ClassReq)) & ".", ColorType.Yellow)
                        End If
                    Else
                        Network.PlayerMsg(index, "This scroll is not connected to a skill, please inform an admin!", ColorType.BrightRed)
                    End If
                Case ItemType.Furniture
                    Network.PlayerMsg(index, "To place furniture, simply click on it in your inventory, then click in your house where you want it.", ColorType.Yellow)

                Case ItemType.Recipe

                    Network.PlayerMsg(index, "Lets learn this recipe :)", ColorType.BrightGreen)
                    ' Get the recipe num
                    n = Item(InvItemNum).Data1
...
#Region "Equipment"
    ...
    Sub PlayerUnequipItem(index As Integer, EqSlot As Integer)
        ...
        If FindOpenInvSlot(index, GetPlayerEquipment(index, EqSlot)) > 0 Then
            ...
            Network.PlayerMsg(index, "You unequip " & CheckGrammar(Item(GetPlayerEquipment(index, EqSlot)).Name), ColorType.Yellow)
            ' remove equipment
...
#Region "Misc"
    ...
    Sub JoinGame(index As Integer)
        ...
        Network.GlobalMsg(String.Format("{0} has joined {1}!", GetPlayerName(index), Configuration.Settings.GameName))

        ' Send an ok to client to start receiving in game data
        Network.SendLoadCharOk(index)
        ...
        Network.SendTotalOnlineTo(index)
        CheckEquippedItems(index)
        Network.SendGameData(index)
        Network.SendInventory(index)
        Network.SendWornEquipment(index)
        Network.SendMapEquipment(index)
        ...
        Network.SendMapNames(index)
        Network.SendHotbar(index)
        Network.SendPlayerSkills(index)
        SendRecipes(index)
        Network.SendStats(index)
        Network.SendJoinMap(index)
        ...
        Network.SendTotalOnlineToAll()

        ' Warp the player to his saved location
        PlayerWarp(index, GetPlayerMap(index), GetPlayerX(index), GetPlayerY(index))

        ' Send welcome messages
        Network.SendWelcome(index)

        ' Send the flag so they know they can start doing stuff
        Network.SendInGame(index)
    ...
    Sub LeftGame(index As Integer)
        ...
        If TempPlayer(index).InGame Then
            ...
            ' Check if the player was in a party, and if so cancel it out so the other player doesn't
            ' continue to get half exp leave party.
            Party_PlayerLeave(index)

            ' cancel any trade they're in
            If TempPlayer(index).InTrade > 0 Then
                tradeTarget = TempPlayer(index).InTrade
                Network.PlayerMsg(tradeTarget, String.Format("{0} has declined the trade.", GetPlayerName(index)), ColorType.BrightRed)
                ' clear out trade
                For i = 1 To MAX_INV
                ...
                Network.SendCloseTrade(tradeTarget)
            End If
            ...
            Network.GlobalMsg(String.Format("{0} has left {1}!", GetPlayerName(index), Configuration.Settings.GameName))

            Console.WriteLine(String.Format("{0} has left {1}!", GetPlayerName(index), Configuration.Settings.GameName))
    ...
    Friend Sub KillPlayer(index As Integer)
        ...
        If exp < 0 Then exp = 0
        If exp = 0 Then
            Network.PlayerMsg(index, "You've lost no experience.", ColorType.BrightGreen)
        Else
            SetPlayerExp(index, GetPlayerExp(index) - exp)
            Network.SendExp(index)
            Network.PlayerMsg(index, String.Format("You've lost {0} experience.", exp), ColorType.BrightRed)
        End If
    ...
    Sub OnDeath(index As Integer)
        ...
        With Map(GetPlayerMap(index))
            ...
            Else
                PlayerWarp(index, Configuration.Settings.StartMap, Configuration.Settings.StartX, Configuration.Settings.StartY)
            End If
        End With
        ...
        Network.SendClearSkillBuffer(index)
    ...
    Friend Sub HandlePlayerKillExperience(Attacker As Integer, Victim As Integer)
        ...
        If exp = 0 Then
            Network.PlayerMsg(Victim, "You've lost no exp.", ColorType.BrightRed)
            Network.PlayerMsg(Attacker, "You've received no exp.", ColorType.BrightBlue)
        Else
            SetPlayerExp(Victim, GetPlayerExp(Victim) - exp)
            Network.SendExp(Victim)
            Network.PlayerMsg(Victim, String.Format("You've lost {0} exp.", exp), ColorType.BrightRed)
...
#Region "Skills"
    ...
    Friend Sub BufferSkill(index As Integer, Skillslot As Integer)
        ...
        If TempPlayer(index).SkillCd(Skillslot) > GetTimeMs() Then
            Network.PlayerMsg(index, "Skill hasn't cooled down yet!", ColorType.Yellow)
            Exit Sub
        End If
        ...
        If GetPlayerVital(index, VitalType.MP) < MPCost Then
            Network.PlayerMsg(index, "Not enough mana!", ColorType.Yellow)
            Exit Sub
        End If
        ...
        If LevelReq > GetPlayerLevel(index) Then
            Network.PlayerMsg(index, "You must be level " & LevelReq & " to use this skill.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        If AccessReq > GetPlayerAccess(index) Then
            Network.PlayerMsg(index, "You must be an administrator to use this skill.", ColorType.BrightRed)
            Exit Sub
        End If
        ...
        If ClassReq > 0 Then ' 0 = no req
            If ClassReq <> GetPlayerClass(index) Then
                Network.PlayerMsg(index, "Only " & CheckGrammar(Classes(ClassReq).Name.Trim) & " can use this skill.", ColorType.Yellow)
                Exit Sub
            End If
        End If
        ...
        Select Case SkillCastType
            ...
            Case 2, 3 ' targeted & targeted AOE
                ...
                If Not Target > 0 Then
                    Network.PlayerMsg(index, "You do not have a target.", ColorType.BrightRed)
                End If
                If TargetType = TargetType.Player Then
                    ...
                    If Not IsInRange(range, GetPlayerX(index), GetPlayerY(index), GetPlayerX(Target), GetPlayerY(Target)) Then
                        Network.PlayerMsg(index, "Target not in range.", ColorType.BrightRed)
                    Else
...
#Region "Bank"
    ...
    Function FindOpenBankSlot(index As Integer, ItemNum As Integer) As Byte
        ...
        If Not Network.IsPlaying(index) Then Exit Function
