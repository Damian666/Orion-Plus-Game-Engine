...
Module S_AutoMap
    ' Automapper System
    ' Version: 1.0
    ' Author: Lucas Tardivo (boasfesta) Map analysis and tips: Richard Johnson, Luan Meireles (Alenzinho)
...
#Region "Loading Functions"
    ...
    Sub AddDetail(prefab As TilePrefab, tileset As Integer, x As Integer, y As Integer, tileType As Integer)
        ...
        detailCount = Detail.GetUpperBound(0) + 1
...
#Region "Incoming Packets"
    ...
    Sub Packet_SaveAutoMap(index As Integer, ByRef data() As Byte)
        ...
        For Prefab = 1 To TilePrefab.Count - 1
            ...
            Ini.PutVar(path, "Prefab" & Prefab, "Layer" & Layer & "Tileset", buffer.ReadInt32)
            Ini.PutVar(path, "Prefab" & Prefab, "Layer" & Layer & "X", buffer.ReadInt32)
            Ini.PutVar(path, "Prefab" & Prefab, "Layer" & Layer & "Y", buffer.ReadInt32)
            Ini.PutVar(path, "Prefab" & Prefab, "Layer" & Layer & "Autotile", buffer.ReadInt32)

            Ini.PutVar(path, "Prefab" & Prefab, "Type", buffer.ReadInt32)
        Next
...
#Region "Outgoing Packets"
    ...
    Sub SendAutoMapper(index As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SAutoMapper)
        ...
        'send xml info
        buffer.WriteString((Ini.GetVar(path, "Resources", "ResourcesNum")))

        For Prefab = 1 To TilePrefab.Count - 1
            For Layer = 1 To LayerType.Count - 1
                If CInt(Ini.GetVar(path, "Prefab" & Prefab, "Layer" & Layer & "Tileset")) > 0 Then
                    buffer.WriteInt32(Layer)
                    buffer.WriteInt32(CInt(Ini.GetVar(path, "Prefab" & Prefab, "Layer" & Layer & "Tileset")))
                    buffer.WriteInt32(CInt(Ini.GetVar(path, "Prefab" & Prefab, "Layer" & Layer & "X")))
                    buffer.WriteInt32(CInt(Ini.GetVar(path, "Prefab" & Prefab, "Layer" & Layer & "Y")))
                    buffer.WriteInt32(CInt(Ini.GetVar(path, "Prefab" & Prefab, "Layer" & Layer & "Autotile")))
                End If
            Next
            buffer.WriteInt32(CInt(Ini.GetVar(path, "Prefab" & Prefab, "Type")))
        Next
...
#Region "Creation Functions"
    ...
    Sub AddTile(prefab As TilePrefab, mapNum As Integer, x As Integer, y As Integer)
        ...
        'DetailFreq = CInt(frmAutoMapper.txtDetail.Text)
        ...
        If prefab = TilePrefab.Grass OrElse prefab = TilePrefab.Sand Then
            If DetailsChecked = True Then
                If Random(1, DetailFreq) = 1 Then
                    ...
                    ReDim details(1)
                    For i = 1 To Detail.GetUpperBound(0)
                        If Detail(i).DetailBase = prefab Then
                            ReDim Preserve details(details.GetUpperBound(0) + 1)
                            details(details.GetUpperBound(0)) = i
                        End If
                    Next i
                    If details.GetUpperBound(0) > 1 Then
                        detailNum = details(Random(2, details.GetUpperBound(0)))
            ...
            For y = 1 To Map(mapNum).MaxY - 1
                If CanPlaceResource(mapNum, x, y) AndAlso CanPlaceResource(mapNum, x - 1, y) AndAlso CanPlaceResource(mapNum, x + 1, y) AndAlso CanPlaceResource(mapNum, x, y - 1) AndAlso CanPlaceResource(mapNum, x, y + 1) Then
                    ...
                    If Random(1, ResourceFreq) = 1 Then
                        resourceNum = CInt(_resources(Random(1, _resources.GetUpperBound(0))))
    ...
    Sub MakeOvergrass(mapNum As Integer)
        ...
        With Map(mapNum)
            ...
            totalOvergrass = Random(CInt(grassCount / 100), CInt(grassCount / 50))
...
SelectMap:
    ...
    Sub PlaceMountain(mapNum As Integer, x As Integer, y As Integer, mountainPrefab As MountainTile)
        ...
        Tile(TilePrefab.Mountain).Layer(2).Y = oldY + (CInt(mountainPrefab / 3))
    ...
    Sub MarkMountain(mapNum As Integer, x As Integer, y As Integer, width As Integer, height As Integer)
        Dim pX As Integer, pY As Integer
        For pX = x - CInt(width / 2) To x + CInt(width / 2)
            For pY = y - CInt(height / 2) To y + CInt(height / 2)
    ...
    Sub MakeMap(mapNum As Integer, prefab As MapPrefab)
        ...
        With Map(mapNum)
            ...
            If prefab = MapPrefab.UpLeftQuarter Then
                tileStartX = CInt(.MaxX / 2) - Random(0, CInt(.MaxX / 4))
                tileStartY = .MaxY
                tileEndX = .MaxX
                tileEndY = CInt(.MaxY / 2) - Random(0, CInt(.MaxY / 4))
                tileX = tileStartX
            ...
            If prefab = MapPrefab.UpBorder Then
                ...
                For x = tileStartX To tileEndX
                    ...
                    Else
                        changed = False
                    End If
                    If tileY < CInt(.MaxY / 4) Then tileY = CInt(.MaxY / 4)
                    If tileY > CInt(.MaxY / 2) + CInt(.MaxY / 4) Then tileY = CInt(.MaxY / 2) + CInt(.MaxY / 4)
                    For y = tileY To .MaxY
            ...
            If prefab = MapPrefab.LeftBorder Then
                ...
                For y = tileStartY To tileEndY
                    ...
                    Else
                        changed = False
                    End If
                    If tileX < CInt(.MaxX / 4) Then tileX = CInt(.MaxX / 4)
                    If tileX > CInt(.MaxX / 2) + CInt(.MaxX / 4) Then tileX = CInt(.MaxX / 2) + CInt(.MaxX / 4)
                    For x = tileX To tileEndX
...
ChangeDir:
            ...
            If x <= 0 Then
                If Map(mapNum).Left > 0 Then
                    If _mapOrientation(Map(mapNum).Left).Prefab = MapPrefab.Common Then
                        ...
                        PaintTile(TilePrefab.Passing, Map(mapNum).Left, CInt(Map(mapNum).MaxX), y, brushX, brushY, , TilePrefab.Grass)
            ...
            If y <= 0 Then
                If Map(mapNum).Up > 0 Then
                    If _mapOrientation(Map(mapNum).Up).Prefab = MapPrefab.Common Then
                        PaintTile(TilePrefab.Passing, mapNum, x, y, brushX, brushY, , TilePrefab.Grass)
                        PaintTile(TilePrefab.Passing, Map(mapNum).Up, x, CInt(Map(mapNum).MaxY), brushX, brushY, , TilePrefab.Grass)
    ...
    Sub MakePaths(mapStart As Integer, size As Integer)
        ...
        If totalMaps Mod 2 = 1 Then
            MakeMapPaths(CInt(totalMaps / 2) + 1)
        Else
            MakeMapPaths(CInt(totalMaps / 2) - (size / 2))
            MakeMapPaths(CInt(totalMaps / 2) - (size / 2) + 1)
            MakeMapPaths(CInt(totalMaps / 2) - (size / 2) + size)
            MakeMapPaths(CInt(totalMaps / 2) - (size / 2) + size + 1)
        End If
