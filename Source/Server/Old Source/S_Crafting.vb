...
#Region "Database"
    ...
    Sub CheckRecipes()
        ...
        For i = 1 To MAX_RECIPE
            If Not File.Exists(Path.Combine(Environment.CurrentDirectory, "data", "recipes", String.Format("recipe{0}.dat", i))) Then
                SaveRecipe(i)
    ...
    Sub SaveRecipe(RecipeNum As Integer)
        ...
        filename = Path.Combine(Environment.CurrentDirectory, "data", "recipes", String.Format("recipe{0}.dat", RecipeNum))
...
#Region "Incoming Packets"
    ...
    Sub Packet_RequestEditRecipes(index As Integer, ByRef data() As Byte)
        ...
        Buffer.WriteInt32(ServerPacket.SRecipeEditor)
...
#Region "Outgoing Packets"
    ...
    Sub SendUpdateRecipeTo(index As Integer, RecipeNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SUpdateRecipe)
    ...
    Sub SendPlayerRecipes(index As Integer)
        ...
        Dim buffer As ByteStream
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SSendPlayerRecipe)
    ...
    Sub SendOpenCraft(index As Integer)
        Dim buffer As ByteStream
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SOpenCraft)
    ...
    Sub SendCraftUpdate(index As Integer, done As Byte)
        Dim buffer As ByteStream
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SUpdateCraft)
...
#Region "Functions"
    ...
    Friend Sub LearnRecipe(index As Integer, RecipeNum As Integer, InvNum As Integer)
        If CheckLearnedRecipe(index, RecipeNum) Then ' we know this one allready
            Network.PlayerMsg(index, "You allready know this recipe!", ColorType.BrightRed)
        Else ' lets learn it
            Player(index).Character(TempPlayer(index).CurChar).RecipeLearned(RecipeNum) = 1

            Network.PlayerMsg(index, "You learned the " & Recipe(RecipeNum).Name & " recipe!", ColorType.BrightGreen)
    ...
    Friend Sub UpdateCraft(index As Integer)
        ...
        If GiveInvItem(index, Recipe(TempPlayer(index).CraftRecipe).MakeItemNum, Recipe(TempPlayer(index).CraftRecipe).MakeItemAmount, True) Then
            ...
            Next
            Network.PlayerMsg(index, "You created " & Item(Recipe(TempPlayer(index).CraftRecipe).MakeItemNum).Name.Trim & " X " & Recipe(TempPlayer(index).CraftRecipe).MakeItemAmount, ColorType.BrightGreen)
        End If
