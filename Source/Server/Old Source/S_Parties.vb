...
#Region "Outgoing Packets"
    ...
    Sub SendDataToParty(partyNum As Integer, ByRef data() As Byte)
        ...
        For i = 1 To Party(partyNum).MemberCount
            If Party(partyNum).Member(i) > 0 Then
                Network.SendDataTo(Party(partyNum).Member(i), data)
            End If
        Next
    End Sub
    ...
    Sub SendPartyInvite(index As Integer, target As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPartyInvite)

        Addlog("Sent SMSG: SPartyInvite", PACKET_LOG)
        Console.WriteLine("Sent SMSG: SPartyInvite")

        buffer.WriteString((Player(target).Character(TempPlayer(target).CurChar).Name.Trim))
    ...
    Sub SendPartyUpdate(partyNum As Integer)
        Dim buffer As New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SPartyUpdate)
    ...
    Sub SendPartyVitals(partyNum As Integer, index As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SPartyVitals)
...
#Region "Incoming Packets"
    ...
    Friend Sub Packet_PartyRquest(index As Integer, ByRef data() As Byte)
        ...
        If Not Network.IsConnected(TempPlayer(index).Target) OrElse Not Network.IsPlaying(TempPlayer(index).Target) Then Exit Sub
...
#End Region
    ...
    Friend Sub PartyMsg(partyNum As Integer, msg As String)
        ...
        For i = 1 To MAX_PARTY_MEMBERS
            ...
            If Party(partyNum).Member(i) > 0 Then
                ...
                If Network.IsConnected(Party(partyNum).Member(i)) AndAlso Network.IsPlaying(Party(partyNum).Member(i)) Then
                    Network.PlayerMsg(Party(partyNum).Member(i), msg, ColorType.BrightBlue)
                End If
            End If
        Next
    End Sub
    ...
    Friend Sub Party_Invite(index As Integer, target As Integer)
        ...
        If Not Network.IsConnected(target) OrElse Not Network.IsPlaying(target) Then Exit Sub
        ...
        If TempPlayer(target).PartyInvite > 0 OrElse TempPlayer(target).TradeRequest > 0 Then
            ...
            Network.PlayerMsg(index, "This player is busy.", ColorType.BrightRed)
            ' exit out early
            Exit Sub
        End If
        ...
        If TempPlayer(target).InParty > 0 Then
            ...
            Network.PlayerMsg(index, "This player is already in a party.", ColorType.BrightRed)
            'exit out early
            Exit Sub
        End If
            ...
            If Party(partyNum).Leader = index Then
                ...
                For i = 1 To MAX_PARTY_MEMBERS
                    If Party(partyNum).Member(i) = 0 Then
                        ...
                        ' let them know
                        Network.PlayerMsg(index, "Invitation sent.", ColorType.Yellow)
                        Exit Sub
                    End If
                Next
                ' no room
                Network.PlayerMsg(index, "Party is full.", ColorType.BrightRed)
                Exit Sub
            Else
                ' not the leader
                Network.PlayerMsg(index, "You are not the party leader.", ColorType.BrightRed)
                Exit Sub
            End If
        Else
    ...
    Friend Sub Party_InviteAccept(index As Integer, target As Integer)
        ...
        If TempPlayer(index).InParty > 0 Then
            ...
            Network.PlayerMsg(index, "Party is full.", ColorType.BrightRed)
            Network.PlayerMsg(target, "Party is full.", ColorType.BrightRed)
            Exit Sub
        Else
    ...
    Friend Sub Party_InviteDecline(index As Integer, target As Integer)
        Network.PlayerMsg(index, String.Format("{0} has declined to join your party.", GetPlayerName(target)), ColorType.BrightRed)
        Network.PlayerMsg(target, "You declined to join the party.", ColorType.Yellow)
        ' clear the invitation
        TempPlayer(target).PartyInvite = 0
    End Sub
    ...
    Friend Sub Party_ShareExp(partyNum As Integer, exp As Integer, index As Integer, mapNum As Integer)
        ...
        For i = 1 To MAX_PARTY_MEMBERS
            ...
            If tmpindex > 0 Then
                If Network.IsConnected(tmpindex) AndAlso Network.IsPlaying(tmpindex) Then
