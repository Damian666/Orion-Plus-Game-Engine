...
#Region "Spawning"
    ...
    Friend Sub SpawnNpc(mapNpcNum As Integer, mapNum As Integer)
        ...
        If npcNum > 0 Then
            ...
            MapNpc(mapNum).Npc(mapNpcNum).Dir = Rand(4)
            ...
            If spawned Then
                buffer.WriteInt32(ServerPacket.SSpawnNpc)
...
#Region "Movement"
    ...
    Friend Function NpcTileIsOpen(mapNum As Integer, x As Integer, y As Integer) As Boolean
        ...
        If PlayersOnMap(mapNum) Then
            For LoopI = 1 To Network.HighIndex
    ...
    Sub NpcMove(mapNum As Integer, MapNpcNum As Integer, Dir As Integer, Movement As Integer)
        ...
        Select Case Dir
            Case DirectionType.Up
                ...
                buffer.WriteInt32(ServerPacket.SNpcMove)
                buffer.WriteInt32(MapNpcNum)
                buffer.WriteInt32(MapNpc(mapNum).Npc(MapNpcNum).X)
                buffer.WriteInt32(MapNpc(mapNum).Npc(MapNpcNum).Y)
                buffer.WriteInt32(MapNpc(mapNum).Npc(MapNpcNum).Dir)
    ...
    Sub NpcDir(mapNum As Integer, MapNpcNum As Integer, Dir As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SNpcDir)
        buffer.WriteInt32(MapNpcNum)
...
#Region "Npcombat"
    Friend Sub TryNpcAttackPlayer(mapnpcnum As Integer, index As Integer)
        ...
        If CanNpcAttackPlayer(mapnpcnum, index) Then
            ...
            If CanPlayerDodge(index) Then
                Network.SendActionMsg(mapNum, "Dodge!", ColorType.Pink, 1, (Player(index).Character(TempPlayer(index).CurChar).X * 32), (Player(index).Character(TempPlayer(index).CurChar).Y * 32))
                Exit Sub
            End If

            If CanPlayerParry(index) Then
                Network.SendActionMsg(mapNum, "Parry!", ColorType.Pink, 1, (Player(index).Character(TempPlayer(index).CurChar).X * 32), (Player(index).Character(TempPlayer(index).CurChar).Y * 32))
                Exit Sub
            End If
            ...
            If CanPlayerBlockHit(index) Then
                Network.SendActionMsg(mapNum, "Block!", ColorType.Pink, 1, (Player(index).Character(TempPlayer(index).CurChar).X * 32), (Player(index).Character(TempPlayer(index).CurChar).Y * 32))
                Exit Sub
            Else
                ...
                If CanNpcCrit(npcnum) Then
                    Damage = Damage * 1.5
                    Network.SendActionMsg(mapNum, "Critical!", ColorType.BrightCyan, 1, (MapNpc(mapNum).Npc(mapnpcnum).X * 32), (MapNpc(mapNum).Npc(mapnpcnum).Y * 32))
                End If
    ...
    Function CanNpcAttackPlayer(MapNpcNum As Integer, index As Integer) As Boolean
        ...
        ' Check for subscript out of range
        If MapNpcNum <= 0 OrElse MapNpcNum > MAX_MAP_NPCS OrElse Not Network.IsPlaying(index) Then
            Exit Function
        End If
        ...
        If Network.IsPlaying(index) Then
            If NpcNum > 0 Then
    ...
    Friend Sub NpcAttackPlayer(MapNpcNum As Integer, Victim As Integer, Damage As Integer)
        ...
        If MapNpcNum <= 0 OrElse MapNpcNum > MAX_MAP_NPCS OrElse Network.IsPlaying(Victim) = False Then Exit Sub
        ...
        If Damage >= GetPlayerVital(Victim, VitalType.HP) Then
            ...
            ' Player is dead
            Network.GlobalMsg(GetPlayerName(Victim) & " has been killed by " & Name)
        Else
    ...
    Sub NpcAttackNpc(mapNum As Integer, Attacker As Integer, Victim As Integer, Damage As Integer)
        ...
        If Damage >= MapNpc(mapNum).Npc(Victim).Vital(VitalType.HP) Then
            Network.SendActionMsg(mapNum, "-" & Damage, ColorType.BrightRed, 1, (MapNpc(mapNum).Npc(Victim).X * 32), (MapNpc(mapNum).Npc(Victim).Y * 32))
            Network.SendBlood(mapNum, MapNpc(mapNum).Npc(Victim).X, MapNpc(mapNum).Npc(Victim).Y)
            ...
            n = Rand(Npc(vNpcNum).DropChance(tmpitem)) + 1
            If n = 1 Then
            ...
            buffer.WriteInt32(ServerPacket.SNpcDead)
    ...
    Friend Sub SpellNpc_Effect(Vital As Byte, increment As Boolean, index As Integer, Damage As Integer, Skillnum As Integer, mapNum As Integer)
        ...
        If Damage > 0 Then
            ...
            Network.SendActionMsg(mapNum, sSymbol & Damage, Colour, ActionMsgType.Scroll, MapNpc(mapNum).Npc(index).X * 32, MapNpc(mapNum).Npc(index).Y * 32)
    ...
    Friend Sub DropNpcItems(mapNum As Integer, MapNpcNum As Integer)
        ...
        Dim tmpitem = Random(1, 5)
        Dim n = Rand(Npc(NpcNum).DropChance(tmpitem)) + 1
...
#Region "Outgoing Packets"
    Sub SendMapNpcsToMap(mapNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SMapNpcData)
...
#Region "Incoming Packets"
    Sub Packet_EditNpc(index As Integer, ByRef data() As Byte)
        ...
        Buffer.WriteInt32(ServerPacket.SNpcEditor)
    ...
    Sub SendNpcs(index As Integer)
        ...
        For i = 1 To MAX_NPCS
            If Npc(i).Name.Trim.Length > 0 Then
    ...
    Sub SendUpdateNpcTo(index As Integer, NpcNum As Integer)
        ...
        buffer = New ByteStream(4)
        buffer.WriteInt32(ServerPacket.SUpdateNpc)
    ...
    Sub SendMapNpcTo(mapNum As Integer, MapNpcNum As Integer)
        ...
        buffer.WriteInt32(ServerPacket.SMapNpcUpdate)
    ...
    Sub SendMapNpcVitals(mapNum As Integer, MapNpcNum As Byte)
        ...
        buffer.WriteInt32(ServerPacket.SMapNpcVitals)
        buffer.WriteInt32(MapNpcNum)
