...
Module S_NetworkSend
    ...
    Sub SendBank(index As Integer)
        ...
        For i = 1 To MAX_BANK
            buffer.WriteInt32(Bank(index).Item(i).Index)
            buffer.WriteInt32(Bank(index).Item(i).Amount)
    ...
    Sub SendTradeUpdate(index As Integer, DataType As Byte)
        ...
        If DataType = 0 Then ' own inventory
            ...
            For i = 1 To MAX_INV
                buffer.WriteInt32(TempPlayer(index).TradeOffer(i).Index)
                buffer.WriteInt32(TempPlayer(index).TradeOffer(i).Amount)

                ' add total worth
                If TempPlayer(index).TradeOffer(i).Index > 0 Then
                    ' currency?
                    If Item(TempPlayer(index).TradeOffer(i).Index).Type = ItemType.Currency OrElse Item(TempPlayer(index).TradeOffer(i).Index).Stackable = 1 Then
                        If TempPlayer(index).TradeOffer(i).Amount = 0 Then TempPlayer(index).TradeOffer(i).Amount = 1
                        totalWorth = totalWorth + (Item(GetPlayerInvItemNum(index, TempPlayer(index).TradeOffer(i).Index)).Price * TempPlayer(index).TradeOffer(i).Amount)
                    Else
                        totalWorth = totalWorth + Item(GetPlayerInvItemNum(index, TempPlayer(index).TradeOffer(i).Index)).Price
                    End If
                End If
            Next
        ElseIf DataType = 1 Then ' other inventory
            ...
            For i = 1 To MAX_INV
                buffer.WriteInt32(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index))
                buffer.WriteInt32(TempPlayer(tradeTarget).TradeOffer(i).Amount)

                ' add total worth
                If GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index) > 0 Then
                    ' currency?
                    If Item(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index)).Type = ItemType.Currency OrElse Item(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index)).Stackable = 1 Then
                        If TempPlayer(tradeTarget).TradeOffer(i).Amount = 0 Then TempPlayer(tradeTarget).TradeOffer(i).Amount = 1
                        totalWorth = totalWorth + (Item(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index)).Price * TempPlayer(tradeTarget).TradeOffer(i).Amount)
                    Else
                        totalWorth = totalWorth + Item(GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index)).Price
                    End If
                End If
            Next
        End If
