...
#Region "Drawing"
    ...
    Friend Sub DrawResource(resource As Integer, dx As Integer, dy As Integer, rec As Rectangle)
        If resource < 1 OrElse resource > NumResources Then Return
        Dim x As Integer
        Dim y As Integer
        ...
        If rec.Width < 0 OrElse rec.Height < 0 Then Return
    ...
    Friend Sub DrawMapResource(resourceNum As Integer)
        ...
        If MapData = False Then Return

        If MapResource(resourceNum).X > Map.MaxX OrElse MapResource(resourceNum).Y > Map.MaxY Then Return

        ' Get the Resource type
        resourceMaster = Map.Tile(MapResource(resourceNum).X, MapResource(resourceNum).Y).Data1

        If resourceMaster = 0 Then Return

        If Resource(resourceMaster).ResourceImage = 0 Then Return
