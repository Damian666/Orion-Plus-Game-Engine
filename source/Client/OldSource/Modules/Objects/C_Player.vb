...
#Region "Attacking"
    Sub CheckAttack()
        ...
        If VbKeyControl Then
            If InEvent = True Then Return
            If SkillBuffer > 0 Then Return ' currently casting a skill, can't attack
            If StunDuration > 0 Then Return ' stunned, can't attack
...
#Region "Data Set & Retrieval"
    ...
    Function GetPlayerInvItemValue(index As Integer, invslot As Integer) As Integer
        ...
        If index > MAX_PLAYERS Then Exit Function
        GetPlayerInvItemValue = PlayerInv(invslot).Amount
    End Function
    ...
    Function GetPlayerX(index As Integer) As Integer
        Return If(index > MAX_PLAYERS, 0, Player(index).X)
    End Function

    Function GetPlayerY(index As Integer) As Integer
        Return If(index > MAX_PLAYERS, 0, Player(index).Y)
    End Function

    Function GetPlayerPosition(index As Integer) As Vector2f
        Return If(index > MAX_PLAYERS, New Vector2f(0, 0),
                  New Vector2f(Player(index).X, Player(index).Y))
    End Function
    ...
    Sub SetPlayerEquipment(index As Integer, invNum As Integer, equipmentSlot As EquipmentType)
        If index < 1 OrElse index > MAX_PLAYERS Then Return
