...
#Region "EventEditor"
    ...
    Sub DeleteEvent(ByVal X As Integer, ByVal Y As Integer)
        ...
        If Not InMapEditor Then Return
        If FrmEditor_Events.Visible = True Then Return
    ...
    Sub ClearEvent(ByVal EventNum As Integer)
        If EventNum > Map.EventCount OrElse EventNum > UBound(Map.MapEvents) Then Return
...
newlist:
    ...
    Public Sub EditEventCommand()
        ...
        If i = -1 Then Return
        If i > UBound(EventList) Then Return
        ...
        If curlist = 0 Then Return
        If curslot = 0 Then Return
        If curlist > TmpEvent.Pages(CurPageNum).CommandListCount Then Return
        If curslot > TmpEvent.Pages(CurPageNum).CommandList(curlist).CommandCount Then Return
...
#Region "Incoming Packets"
    ...
    Sub Packet_EventMove(ByRef data() As Byte)
        ...
        movementSpeed = buffer.ReadInt32
        If id > Map.CurrentEvents Then Return
    ...
    Sub Packet_EventDir(ByRef data() As Byte)
        ...
        i = buffer.ReadInt32
        dir = buffer.ReadInt32
        If i > Map.CurrentEvents Then Return
...
#Region "Drawing..."
    ...
    Friend Sub DrawEvents()
        ...
        If Map.EventCount <= 0 Then Return
        For i = 1 To Map.EventCount
            width = 32
            height = 32
            x = Map.Events(i).X * 32
            ...
            x = ConvertMapX(x)
            y = ConvertMapY(y)
            If i > Map.EventCount Then Return
            If 1 > Map.Events(i).PageCount Then Return
...
nextevent:
    ...
    Friend Sub DrawEvent(id As Integer) ' draw on map, outside the editor
        ...
        If Map.MapEvents(id).Visible = 0 Then Return

        Select Case Map.MapEvents(id).GraphicType
            Case 0
                Return
            Case 1
                If Map.MapEvents(id).GraphicNum <= 0 OrElse Map.MapEvents(id).GraphicNum > NumCharacters Then Return
                ...
                End If
            Case 2
                If Map.MapEvents(id).GraphicNum < 1 OrElse Map.MapEvents(id).GraphicNum > NumTileSets Then Return
...
#Region "Misc"
    ...
    Sub ProcessEventMovement(id As Integer)
        ...
        If id > Map.EventCount Then Return
        If id > Map.MapEvents.Length Then Return
