...
Module C_Network_Send
    ...
    Friend Sub SendDropItem(invNum As Integer, amount As Integer)
        ...
        If InBank OrElse InShop Then Return

        ' do basic checks
        If invNum < 1 OrElse invNum > MAX_INV Then Return
        If PlayerInv(invNum).Index < 1 OrElse PlayerInv(invNum).Index > MAX_ITEMS Then Return
        If Item(GetPlayerInvItemNum(Myindex, invNum)).Type = ItemType.Currency OrElse Item(GetPlayerInvItemNum(Myindex, invNum)).Stackable = 1 Then
            If amount < 1 OrElse amount > PlayerInv(invNum).Amount Then Return
        End If
