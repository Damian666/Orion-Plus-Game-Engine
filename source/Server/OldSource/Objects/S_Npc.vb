...
#Region "Spawning"
    ...
    Friend Sub SpawnNpc(mapNpcNum As Integer, mapNum As Integer)
        ...
        If mapNpcNum <= 0 OrElse mapNpcNum > MAX_MAP_NPCS OrElse mapNum <= 0 OrElse mapNum > MAX_CACHED_MAPS Then Return

        npcNum = Map(mapNum).Npc(mapNpcNum)

        If npcNum > 0 Then
            If Not Npc(npcNum).SpawnTime = Time.Instance.TimeOfDay AndAlso Not Npc(npcNum).SpawnTime = 4 Then Return
...
#Region "Npcombat"
    ...
    Friend Sub NpcAttackPlayer(MapNpcNum As Integer, Victim As Integer, Damage As Integer)
        ...
        If MapNpcNum <= 0 OrElse MapNpcNum > MAX_MAP_NPCS OrElse IsPlaying(Victim) = False Then Return
    ...
    Sub NpcAttackNpc(mapNum As Integer, Attacker As Integer, Victim As Integer, Damage As Integer)
        ...
        If Attacker <= 0 OrElse Attacker > MAX_MAP_NPCS Then Return
        If Victim <= 0 OrElse Victim > MAX_MAP_NPCS Then Return
        ...
        If aNpcNum <= 0 Then Return
        If vNpcNum <= 0 Then Return
    ...
    Friend Sub BufferNpcSkill(mapNum As Integer, MapNpcNum As Integer, skillslot As Integer)
        ...
        If skillslot <= 0 OrElse skillslot > MAX_NPC_SKILLS Then Return
        ...
        If MapNpc(mapNum).Npc(MapNpcNum).Vital(VitalType.MP) < MPCost Then Return
