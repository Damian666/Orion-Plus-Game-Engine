...
#Region "Map Items"
    ...
    Sub SpawnItem(itemNum As Integer, ItemVal As Integer, mapNum As Integer, x As Integer, y As Integer)
        ...
        If itemNum < 1 OrElse itemNum > MAX_ITEMS OrElse mapNum <= 0 OrElse mapNum > MAX_CACHED_MAPS Then Return
    ...
    Sub SpawnItemSlot(MapItemSlot As Integer, itemNum As Integer, ItemVal As Integer, mapNum As Integer, x As Integer, y As Integer)
        ...
        ' Check for subscript out of range
        If MapItemSlot <= 0 OrElse MapItemSlot > MAX_MAP_ITEMS OrElse itemNum < 0 OrElse itemNum > MAX_ITEMS OrElse mapNum <= 0 OrElse mapNum > MAX_CACHED_MAPS Then Return
    ...
    Sub SpawnMapItems(mapNum As Integer)
        ...
        ' Check for subscript out of range
        If mapNum <= 0 OrElse mapNum > MAX_CACHED_MAPS Then Return
...
#Region "Incoming Packets"
    ...
    Sub Packet_SaveItem(index As Integer, ByRef data() As Byte)
        ...
        If n < 0 OrElse n > MAX_ITEMS Then Return
    ...
    Sub Packet_DropItem(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(index).InBank OrElse TempPlayer(index).InShop Then Return

        ' Prevent hacking
        If InvNum < 1 OrElse InvNum > MAX_INV Then Return
        If GetPlayerInvItemNum(index, InvNum) < 1 OrElse GetPlayerInvItemNum(index, InvNum) > MAX_ITEMS Then Return
        If Item(GetPlayerInvItemNum(index, InvNum)).Type = ItemType.Currency OrElse Item(GetPlayerInvItemNum(index, InvNum)).Stackable = 1 Then
            If Amount < 1 OrElse Amount > GetPlayerInvItemValue(index, InvNum) Then Return
        End If
