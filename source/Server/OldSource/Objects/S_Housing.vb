...
#Region "Incoming Packets"
    ...
    Sub Packet_PlaceFurniture(index As Integer, ByRef data() As Byte)
        ...
        ItemNum = Player(index).Character(TempPlayer(index).CurChar).Inv(invslot).Index

        ' Prevent hacking
        If ItemNum < 1 OrElse ItemNum > MAX_ITEMS Then Return
        ...
        If Player(index).Character(TempPlayer(index).CurChar).InHouse = index Then
            If Item(ItemNum).Type = ItemType.Furniture Then
                ...
                If x < 0 OrElse x > Map(GetPlayerMap(index)).MaxX Then Return
                If y < 0 OrElse y > Map(GetPlayerMap(index)).MaxY Then Return
                ...
                If widthoffset > 0 Then
                    ...
                    For x = x1 To x1 + widthoffset
                        For y = y1 To y1 - Item(ItemNum).FurnitureHeight + 1 Step -1
                            If Map(GetPlayerMap(index)).Tile(x, y).Type = TileType.Blocked Then Return
