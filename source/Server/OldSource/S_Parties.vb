...
#Region "Incoming Packets"
    ...
    Friend Sub Packet_PartyRquest(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(index).Target = index Then Return
        ' make sure it's a valid target
        If TempPlayer(index).TargetType <> TargetType.Player Then Return

        ' make sure they're connected and on the same map
        If Not Socket.IsConnected(TempPlayer(index).Target) OrElse Not IsPlaying(TempPlayer(index).Target) Then Return
        If GetPlayerMap(TempPlayer(index).Target) <> GetPlayerMap(index) Then Return
...
#End Region
    ...
    Friend Sub Party_Invite(index As Integer, target As Integer)
        ...
        If Not Socket.IsConnected(target) OrElse Not IsPlaying(target) Then Return
