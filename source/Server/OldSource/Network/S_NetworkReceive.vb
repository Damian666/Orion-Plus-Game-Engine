...
Module S_NetworkReceive
    ...
    Private Sub Packet_AddChar(index As Integer, ByRef data() As Byte)
        ...
        If Not IsPlaying(index) Then
            ...
            If (Sex < modEnumerators.SexType.Male) OrElse (Sex > modEnumerators.SexType.Female) Then Return

            If Classes < 1 OrElse Classes > Max_Classes Then Return
    ...
    Private Sub Packet_PlayerMove(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(index).GettingMap = True Then Return
        ...
        ' Prevent hacking
        If Dir < DirectionType.Up OrElse Dir > DirectionType.Right Then Return

        If movement < 1 OrElse movement > 2 Then Return
    ...
    Sub Packet_Attack(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(index).SkillBuffer > 0 Then Return

        ' can't attack whilst stunned
        If TempPlayer(index).StunDuration > 0 Then Return
        ...
        Select Case GetPlayerDir(index)
            Case DirectionType.Up
                ...
                If GetPlayerY(index) = 0 Then Return
                x = GetPlayerX(index)
                y = GetPlayerY(index) - 1
            Case DirectionType.Down

                If GetPlayerY(index) = Map(GetPlayerMap(index)).MaxY Then Return
                x = GetPlayerX(index)
                y = GetPlayerY(index) + 1
            Case DirectionType.Left

                If GetPlayerX(index) = 0 Then Return
                x = GetPlayerX(index) - 1
                y = GetPlayerY(index)
            Case DirectionType.Right

                If GetPlayerX(index) = Map(GetPlayerMap(index)).MaxX Then Return
                x = GetPlayerX(index) + 1
                y = GetPlayerY(index)
        End Select
    ...
    Sub Packet_WarpMeTo(index As Integer, ByRef data() As Byte)
        ...
        ' Prevent hacking
        If GetPlayerAccess(index) < AdminType.Mapper Then Return
    ...
    Sub Packet_WarpTo(index As Integer, ByRef data() As Byte)
        ...
        ' Prevent hacking
        If n < 0 OrElse n > MAX_CACHED_MAPS Then Return
    ...
    Sub Packet_DestroyBans(index As Integer, ByRef data() As Byte)
        ...
        ' Prevent hacking
        If GetPlayerAccess(index) < AdminType.Creator Then Return
    ...
    Sub Packet_EditShop(index As Integer, ByRef data() As Byte)
        ...
        ' Prevent hacking
        If GetPlayerAccess(index) < AdminType.Developer Then Return
    ...
    Sub Packet_SaveShop(index As Integer, ByRef data() As Byte)
        ...
        ' Prevent hacking
        If ShopNum < 0 OrElse ShopNum > MAX_SHOPS Then Return
    ...
    Sub Packet_PlayerSearch(index As Integer, ByRef data() As Byte)
        ...
        If x < 0 OrElse x > Map(GetPlayerMap(index)).MaxX OrElse y < 0 OrElse y > Map(GetPlayerMap(index)).MaxY Then Return
    ...
    Sub Packet_SwapInvSlots(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(index).InTrade > 0 OrElse TempPlayer(index).InBank OrElse TempPlayer(index).InShop Then Return
    ...
    Sub Packet_TrainStat(index As Integer, ByRef data() As Byte)
        ...
        ' check points
        If GetPlayerPOINTS(index) = 0 Then Return
    ...
    Sub Packet_ForgetSkill(index As Integer, ByRef data() As Byte)
        ...
        If skillslot < 1 OrElse skillslot > MAX_PLAYER_SKILLS Then Return
    ...
    Sub Packet_BuyItem(index As Integer, ByRef data() As Byte)
        ...
        If shopnum < 1 OrElse shopnum > MAX_SHOPS Then Return
    ...
    Sub Packet_SellItem(index As Integer, ByRef data() As Byte)
        ...
        ' if invalid, exit out
        If invSlot < 1 OrElse invSlot > MAX_INV Then Return

        ' has item?
        If GetPlayerInvItemNum(index, invSlot) < 1 OrElse GetPlayerInvItemNum(index, invSlot) > MAX_ITEMS Then Return
    ...
    Sub Packet_TradeInvite(index As Integer, ByRef data() As Byte)
        ...
        If tradetarget <= 0 OrElse tradetarget > MAX_PLAYERS Then Return
    ...
    Sub Packet_TradeInviteAccept(index As Integer, ByRef data() As Byte)
        ...
        If TempPlayer(tradetarget).TradeRequest = index Then
            ...
            For i = 1 To MAX_INV
                TempPlayer(index).TradeOffer(i).Index = 0
                TempPlayer(index).TradeOffer(i).Amount = 0
    ...
    Sub Packet_AcceptTrade(index As Integer, ByRef data() As Byte)
        ...
        For i = 1 To MAX_INV
            ' player
            If TempPlayer(index).TradeOffer(i).Index > 0 Then
                itemNum = Player(index).Character(TempPlayer(index).CurChar).Inv(TempPlayer(index).TradeOffer(i).Index).Index
                If itemNum > 0 Then
                    ' store temp
                    tmpTradeItem(i).Index = itemNum
                    tmpTradeItem(i).Amount = TempPlayer(index).TradeOffer(i).Amount
                    ' take item
                    TakeInvSlot(index, TempPlayer(index).TradeOffer(i).Index, tmpTradeItem(i).Amount)
                End If
            End If
            ' target
            If TempPlayer(tradeTarget).TradeOffer(i).Index > 0 Then
                itemNum = GetPlayerInvItemNum(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index)
                If itemNum > 0 Then
                    ' store temp
                    tmpTradeItem2(i).Index = itemNum
                    tmpTradeItem2(i).Amount = TempPlayer(tradeTarget).TradeOffer(i).Amount
                    ' take item
                    TakeInvSlot(tradeTarget, TempPlayer(tradeTarget).TradeOffer(i).Index, tmpTradeItem2(i).Amount)
                End If
            End If
        Next
        ...
        For i = 1 To MAX_INV
            ' player
            If tmpTradeItem2(i).Index > 0 Then
                ' give away!
                GiveInvItem(index, tmpTradeItem2(i).Index, tmpTradeItem2(i).Amount, False)
            End If
            ' target
            If tmpTradeItem(i).Index > 0 Then
                ' give away!
                GiveInvItem(tradeTarget, tmpTradeItem(i).Index, tmpTradeItem(i).Amount, False)
            End If
        Next
    ...
    Sub Packet_TradeItem(index As Integer, ByRef data() As Byte)
        ...
        If itemnum <= 0 OrElse itemnum > MAX_ITEMS Then Return

        ' make sure they have the amount they offer
        If amount < 0 OrElse amount > GetPlayerInvItemValue(index, invslot) Then Return
        ...
        If Item(itemnum).Type = ItemType.Currency OrElse Item(itemnum).Stackable = 1 Then
            ...
            For i = 1 To MAX_INV
                ...
                If TempPlayer(index).TradeOffer(i).Index = invslot Then
                    ' add amount
                    TempPlayer(index).TradeOffer(i).Amount = TempPlayer(index).TradeOffer(i).Amount + amount

                    ' clamp to limits
                    If TempPlayer(index).TradeOffer(i).Amount > GetPlayerInvItemValue(index, invslot) Then
                        TempPlayer(index).TradeOffer(i).Amount = GetPlayerInvItemValue(index, invslot)
                    End If
        ...
        For i = 1 To MAX_INV
            ...
            End If
        Next
        TempPlayer(index).TradeOffer(emptyslot).Index = invslot
        TempPlayer(index).TradeOffer(emptyslot).Amount = amount
    ...
    Sub Packet_UntradeItem(index As Integer, ByRef data() As Byte)
        ...
        If tradeslot <= 0 OrElse tradeslot > MAX_INV Then Return
        If TempPlayer(index).TradeOffer(tradeslot).Index <= 0 Then Return

        TempPlayer(index).TradeOffer(tradeslot).Index = 0
        TempPlayer(index).TradeOffer(tradeslot).Amount = 0
